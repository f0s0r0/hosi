...
$dateArray = getAllDatesBetweenTwoDates($fromDate, $toDate);
  ...
  foreach($dateArray as $date){
	  ...
	  $query2 = "select registrationdate,count(*) as dd FROM master_visitentry where registrationdate = '$date' "; 
...
/*$ymaxrange = max($registrationdate1);
$yminrange = min(array_filter($registrationdate1));
...
$g = new graph();
$g->title( 'OP Visit','{font-size: 20px;}' );
