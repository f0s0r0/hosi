...
<tr>
<td colspan="12" align="center" valign="middle"><strong>Consultation Reconcile</strong></td>
</tr>
...
<?php 
$query8 = "SELECT transactionmode,patientvisitcode,billnumber,accountname,cashamount,chequeamount,cardamount,onlineamount,creditamount,billdate,bankcode,cashcode,mpesacode FROM `billing_consultation` WHERE `billdate` BETWEEN '$ADate1' AND '$ADate2' order by auto_number";
...
<td align="left" valign="middle"><?= $transactionmode; ?></td>
	...
	if($parentid != '')
	{
		if($parentid == '18' || $parentid == '19' || $parentid == '20')
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(`creditamount`) as bankamount FROM `billing_consultation` WHERE `mpesacode` = '$id' AND `billdate` BETWEEN '$ADate1' AND '$ADate2'
								UNION ALL SELECT SUM(`cashamount`) as bankamount FROM `billing_consultation` WHERE `cashcode` = '$id' AND `billdate` BETWEEN '$ADate1' AND '$ADate2'
		...
		else if($parentid == '1')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT SUM(`consultation`) as income FROM `billing_consultation` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'";
