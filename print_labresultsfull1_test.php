...
<?php 
if($itemcode==""){
$query341 = "select * from consultation_lab where patientvisitcode= '$visitcode' and resultentry = 'completed'  order by auto_number";

}else{
$query341 = "select * from consultation_lab where billnumber = '$billnumber' and resultentry = 'completed' and labitemcode='$itemcode'   order by auto_number";
}
...
$query38="select parametercode as referencecode,parametername as referencename from pending_test_orders where testcode='$res616itemcode' and patientcode = '$patientcode' and visitcode = '$visitcode' order by auto_number asc";
   $exec38=mysql_query($query38);
  $num38=mysql_num_rows($exec38);
if($num38==0){

	$query38="select *,'med360' as referencecode  from master_labreference where itemcode = '$res616itemcode' and status <> 'deleted' 
...
group by referencename ";
	...
	while($res38=mysql_fetch_array($exec38))
	{
	 ...
	 $referencecode1=$res38['referencecode'];
	
	$query32="select * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and (referencename = '$referencename1' OR referencename = '$referencecode1')  and itemcode = '$res616itemcode'  order by auto_number DESC";
