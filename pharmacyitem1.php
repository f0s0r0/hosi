...
if ($frmflag1 == 'frmflag1')
{
	...
	$hidden_mtiba_code = $_REQUEST['hidden_mtiba_code'];
	...
	$query4 = "select * from master_tax where auto_number = '$taxanum'";// and cstid='$custid' and cstname='$custname'";
		...
		if ($res2 == 0)
		{
			$query1 = "insert into master_medicine (itemcode, itemname, categoryname, unitname_abbreviation,packagename,rateperunit, expiryperiod, taxanum, taxname, ipaddress, updatetime, description, purchaseprice,pkg,genericname,exclude,minimumstock,maximumstock,rol,roq,ipmarkup,spmarkup,formula,disease,transfertype,type,inventorytype,mtiba_code) 
			values ('$itemcode', '$itemname', '$categoryname', '$res4packagename','$res4packagename', '$rateperunit', '$expiryperiod', '$taxanum', '$res4taxname', '$ipaddress', '$updatedatetime','$description', '$purchaseprice','$pkg','$genericname','$exclude','$minimumstock','$maximumstock','$rol','$roq','$ipmarkup','$spmarkup','$formula','$disease','$transfertype','$medtype','$inventorytype','$hidden_mtiba_code')";
			$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
...
function additem1process1()
{
	...
	*/
	if (document.form1.hidden_mtiba_code.value == "")
	{
		alert ("Pleae Select Mtiba Item From The List.");
		document.form1.search_mtiba_code.focus();
		return false;
	}
...
$(function(){
// autosearch mtiba item code start by ganesh
	$('#search_mtiba_code').autocomplete({		
	source:'ajax_mtiba_code.php?get=DO-', 	
	minLength:1,
	delay: 0,
	html: true, 	
	select: function(event,ui){
				var mtibaname=ui.item.value;
				var mtibacode=ui.item.item_code;
				$('#hidden_mtiba_code').val(mtibacode);
	},
	change: function(event,ui){
			...
			$('#hidden_mtiba_code').val('');
	});	
	
// autosearch mtiba item code end by ganesh		
});
</script>
...
-->					  
      ...
      <?php }?><input type="hidden" name="locationcount" value="<?php echo  $incr;?>">
						...
						<tr>
							<td align="left" valign="top"  bgcolor="#E0E0E0" class="bodytext3">Mtiba Item </td>
							<td align="left" valign="top"  bgcolor="#E0E0E0">
								<input type="text" name="search_mtiba_code" id="search_mtiba_code" value="" style="border: 1px solid #001E6A" size="60">
								<input type="hidden" name="hidden_mtiba_code" id="hidden_mtiba_code" value="">
								<!-- // by ganesh -->
							</td>					
					</tr>		
					 <tr>
                    ...
                    <tbody>
                      <tr bgcolor="#011E6A">
						...
						if($searchflag1 != 'searchflag1'){
							...
							*/
							$query111 = "select * from $tbl_name where status <> 'deleted'";
							$exec111 = mysql_query($query111) or die ("Error in Query111".mysql_error());
	  ...
	  if ($searchflag1 == 'searchflag1')
	  {
	    ...
	    $query1 = "select * from master_medicine where itemname like '%$search1%' and status <> 'deleted' order by auto_number desc ";
		$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
		while ($res1 = mysql_fetch_array($exec1))
		{
	...
	$query1 = "select * from master_medicine where status <> 'deleted' order by auto_number desc LIMIT $start, $limit";
	  ...
	  if ($searchflag2 == 'searchflag2')
	  {
	    ...
	    $query1 = "select * from master_medicine where itemname like '%$search2%' and status = 'deleted' order by auto_number desc LIMIT 100";
		$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
		while ($res1 = mysql_fetch_array($exec1))
		{
		$itemcode = $res1["itemcode"];
