...
$docnumber=$billnumbercode;
		...
		$orderedbyname = $res24['employeename'];
			$qryresult = "select a.result,date(a.resultdatetime) as recorddate, time(a.resultdatetime) as recordtime,a.parametername,a.parametercode from pending_test_orders as a join master_test_parameter as  b  on( a.testcode = b.labcode and a.parametercode = b.parametercode) where a.patientcode ='$patientcode' and a.visitcode = '$visitcode' and a.result <> '' and  a.testcode ='$labcode' and a.sample_id = '$docno'";
		...
		$query64a = "select * from master_labreference where itemcode='$labcode' and referencename = '$refname' and status <> 'deleted' group by referencename";
			   $exec52=mysql_query($query64a);
				  $res52=mysql_fetch_array($exec52);

				  $unit = $res52['referenceunit'];
				   $referencevalue = $res52['referencerange'];
 ...
 $query29=mysql_query("update consultation_lab set resultentry='$status1',resultdoc='$docnumber' where labitemcode='$labcode' and patientvisitcode='$visitcode' and docnumber='$sampleid'") or die(mysql_error());
   ...
   $query44 = "update consultation_lab set publishstatus='completed',publishdatetime='".date('Y-m-d h:i:s')."' where resultdoc='$docnumber'";
		...
		$res24 = mysql_fetch_array($exec24);
		$orderedbyname = $res24['employeename'];
			$qryresult = "select a.result,date(a.resultdatetime) as recorddate, time(a.resultdatetime) as recordtime,a.parametername,a.parametercode from pending_test_orders as a join master_test_parameter as  b  on( a.testcode = b.labcode and a.parametercode = b.parametercode) where a.patientcode = '$patientcode' and a.visitcode = '$visitcode' and a.result <> '' and a.testcode ='$labcode' and a.sample_id = '$docno'";
		$execresult = mysql_query($qryresult) or die(mysql_error());
		while($resresults = mysql_fetch_array($execresult))
		{
		$resultvalue = $resresults['result'];
