...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$accountcode = $splitdelimiter[2];
			$accountname = $splitdelimiter[3];
			$accountname = strtoupper($accountname);
			$itemname = strtoupper($itemname);
			
			preg_match ('/[!,^,+,=,[,],;,,,{,},|,\,<,>,?,~]/', $accountname);
			...
			if ($accountcode != '')
			{
			$query2 = "insert into pharma_insurance(itemcode,itemname,code,accountname,ipaddress,recordupdate,username) values ('$itemcode','$itemname','$accountcode','$accountname','$ipaddress','$updatedatetime','$username')";
	...
	header("location:pharmaexclusionupload1.php?st=success");
}
?>
<style type="text/css">
<!--
