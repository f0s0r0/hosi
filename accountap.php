       ...
       <tr>
        <td valign="top"><table width="701" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
            ...
            <?php
				...
				$query267 = "select accountname,auto_number,id from master_accountname where accountssub IN ($idbuild)";
				...
				{  
					...
					$querycr1 = "SELECT SUM(`transactionamount`) as payables FROM `master_transactionpharmacy` WHERE `suppliercode` = '$id' AND `transactiontype` = 'PURCHASE' AND particulars <> '' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
								 ...
								 UNION ALL SELECT SUM(depreciation) as payables FROM assets_depreciation WHERE accdepreciationledgercode = '$id' AND entrydate BETWEEN '$ADate1' AND '$ADate2'
								 UNION ALL SELECT SUM(`totalamount`) as payables FROM `purchase_details` WHERE `accdepreciation_code` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' and recordstatus <> 'deleted'";
					...
					<a href="detail_accountap.php?id=<?= $id; ?>&&ADate1=<?= $ADate1; ?>&&ADate2=<?= $ADate2; ?>" target="_blank"><?php echo $colorloopcount; ?>. <?php echo $accountsmain2; ?></a></strong></td>
