...
if($servicename != "")
   { 
   $query26="insert into ipprocess_service(patientname,patientcode,patientvisitcode,recorddate,recordtime,itemcode,itemname,acknowledge,refund,docnumber,locationname,locationcode)values('$patientname','$patientcode',
   '$visitcode','$dateonly','$timeonly','$itemcode','$servicename','$status','$status1','$docnumber','".$locationnameget."','".$locationcodeget."')";
   $exec26=mysql_query($query26) or die(mysql_error());
   mysql_query("update ipconsultation_services set process='$status',servicerefund='$status1',docnumber='$docnumber' where patientvisitcode='$visitcode' and servicesitemname='$servicename' ");
...
// header("location:ipservicelist.php");
// exit();
}
...
$Patientname=$res65['patientfullname'];
  ...
  $locationnameget = $res65['locationname'];
...
<form name="frmsales" id="frmsales" method="post" action="ipprocessservice.php" onKeyDown="return disableEnterKey(event)">
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$sno = '';
			$totalamount=0;			
			$query61 = "select * from ipconsultation_services where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and process='pending' group by servicesitemname";
