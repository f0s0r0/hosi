...
$query312 = "select auto_number, monthly from master_payrollcomponent where recordstatus <> 'deleted' and typecode = '10'";
$exec312 = mysql_query($query312) or die ("Error in Query312".mysql_error());
while ($res312 = mysql_fetch_array($exec312))
{
	$auto_number = $res312['auto_number'];
	...
	$res31componentanum = $auto_number;
	$res31monthly = $res312['monthly'];
...
$query671 = "select auto_number, monthly from master_payrollcomponent where recordstatus <> 'deleted' and typecode = '10' and taxinclude = 'No'";
...
$query391 = "select * from master_payrollcomponent where recordstatus <> 'deleted'";
$exec391 = mysql_query($query391) or die ("Error in Query391".mysql_error());
while ($res391 = mysql_fetch_array($exec391))
{
	$autonumber = $res391['auto_number'];
	
	$query39 = "select `$autonumber` as componentamount from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
	$exec39 = mysql_query($query39) or die ("Error in Query39".mysql_error());
	$res39 = mysql_fetch_array($exec39);
	$res39componentanum = $autonumber;
...
//$query88 = mysql_query("select amount as componentamount from master_nssf where status <> 'deleted'");
...
if($res71componentname != '')
{
	...
	{
	$nssfamount = $res88['componentamount'];
	}
}
...
$query661 = "select * from master_payrollcomponent where recordstatus <> 'deleted' and typecode = '20'";
$exec661 = mysql_query($query661) or die ("Error in Query661".mysql_error());
while ($res661 = mysql_fetch_array($exec661))
{
	$componentanum = $res661['auto_number'];
	
	$query66 = "select `$componentanum` as componentamount from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
	$exec66 = mysql_query($query66) or die ("Error in Query66".mysql_error());
	$res66 = mysql_fetch_array($exec66);
	$res66componentanum = $componentanum;
	//$res66componentamount = $res66['componentamount'];
	$res66monthly = $res661['monthly'];
	if($res66monthly == 'Yes')
	{
...
$totaldeductions = $totaldeductions + $nhifamount + $totalpayeeamount + $nssfamount;
