...
$query2 = "select * from master_doctor where status like '%$status%' group by doctorcode order by doctorname";// desc limit 0, 100";
      ...
      <tr>
        <td><table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			if ($frmflag1 == 'frmflag1')
			{
			  ...
			  $query2 = "select * from master_doctor where doctorname like '%$searchdoctor%' and status like '%$status%' group by doctorcode";
			}
			else
			{
			  $query2 = "select * from master_doctor where status like '%$status%' group by doctorcode order by doctorname";// desc limit 0, 100";
