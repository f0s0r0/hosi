...
if ($frm1submit1 == 'frm1submit1')
{
	...
	foreach($_POST['referralanum'] as $key => $value)
	{
	$referralanum = $_POST['referralanum'][$key];
	$referralrate = $_POST['referralrate'][$key];
	
	$query63 = "update consultation_referal set referalrate='$referralrate' where auto_number='$referralanum' and patientvisitcode='$visitcode'";
	$exec63 = mysql_query($query63) or die(mysql_error());
	}
	
	
		header("location:amend_pending_externalreferral.php");
...
$data=mysql_query("select auto_number,patientcode,patientvisitcode,patientname,refno,referalcode,referalname,referalrate from consultation_referal where auto_number='$auto_number'") or die("Error in data".mysql_error());
...
if($medicinecode<>'')
{
 $query="insert into amendment_details (patientcode,visitcode,patientname,refno,itemcode,itemname,rate,amenddate,amendtime,amendfrom,amendby,ipaddress) values ('$patientcode','$visitcode','$patientname','$refno','$medicinecode','$medicinename','$rate','$date','$time','referal','$username','$ipaddress')";
mysql_query($query)or die("Error in query".mysql_error());
}


mysql_query("delete from consultation_referal where auto_number='$auto_number' and patientvisitcode='$visit'");
}
...
<form name="form1" id="frmsales" method="post" action="amendexternalreferral.php" onKeyDown="return disableEnterKey(event)">
...
-->
  <tr>
	...
	$status='pending';
			...
			$query17 = "select * from consultation_referal where patientvisitcode='$visitcode' and patientcode='$patientcode'";
			...
			while ($res17 = mysql_fetch_array($exec17))
			{
				...
				$query18 = "select * from consultation_referal where patientvisitcode='$visitcode' and patientcode='$patientcode' order by auto_number limit 0,1";
			...
			?>
			 ...
			 <input type="hidden" name="referralanum[]" id="referralanum" value="<?php echo $auto_number; ?>">
			    <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="referralrate[]" id="referralrate" style="text-align:right" size="8" value="<?php echo $pharmitemrate; ?>"></div></td>
				 <td class="bodytext31" valign="center"  align="left"><div align="center"><a onClick="return deletevalid()" href="amendexternalreferral.php?delete=<?php echo $paharmitemname; ?>&&auto_number=<?php echo $auto_number; ?>&&patientcode=<?php echo $patientcode; ?>&&visitcode=<?php echo $visitcode; ?>">Delete</a></div></td>
