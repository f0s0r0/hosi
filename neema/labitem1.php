	...
	for($i=1; $i<=$loccountloop; $i++)
{
	...
	$query12 = "select locationname from master_location where locationcode='".$location."'";
	...
	if ($length1<25 && $length2<255)
	{
		...
		{	
			...
			if($res25 == 0)
			{
				...
				{
					$referencetable = $res25["referencetable"];
					$query61 = "insert into `$referencetable` (itemcode, itemname, categoryname, itemname_abbreviation, rateperunit, expiryperiod, taxanum, taxname, ipaddress, updatetime, description, purchaseprice, referencevalue,ipmarkup,sampletype,location,locationname,referencename,referenceunit,referencerange,criticallow,criticalhigh,displayname) 
				values ('$itemcode', '$itemname', '$categoryname', '$itemname_abbreviation', '', '', '', '', '$ipaddress', '$updatedatetime','manual', '', '$referencevalue','$ipmarkup','$sampletype','','','$reference','$units','$range','$criticallow','$criticalhigh','$displayname')";
	...
	$query24 = "select * from master_lab where itemcode = '$itemcode' and location='".$loccodeget."'";// or itemname = '$itemname'";
		...
		if($res24 == 0)
	{
		$query1 = "insert into master_lab (itemcode, itemname, categoryname, itemname_abbreviation, rateperunit, expiryperiod, taxanum, taxname, ipaddress, updatetime, description, purchaseprice, referencevalue,ipmarkup,  sampletype,location,externallab,externalrate,pkg,locationname) 
			values ('$itemcode', '$itemname', '$categoryname', '$itemname_abbreviation', '$locrateget', '$expiryperiod', '$taxanum', '$res4taxname', '$ipaddress', '$updatedatetime','manual', '$purchaseprice', '$referencevalue','$ipmarkup','$sampletype','$loccodeget','$externallab','$extrate','".$pkg."','".$searchlocationname."')";
			//echo $query1;
			$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
			...
			while ($res25 = mysql_fetch_array($exec25))
			{
				...
				$querytmp1 = "insert into `$templatename` (itemcode, itemname, categoryname, itemname_abbreviation, rateperunit, expiryperiod, taxanum, taxname, ipaddress, updatetime, description, purchaseprice, referencevalue,ipmarkup,  sampletype,location,externallab,externalrate,pkg,locationname) 
			values ('$itemcode', '$itemname', '$categoryname', '$itemname_abbreviation', '$locrateget', '$expiryperiod', '$taxanum', '$res4taxname', '$ipaddress', '$updatedatetime','manual', '$purchaseprice', '$referencevalue','$ipmarkup','$sampletype','$loccodeget','$externallab','$extrate','".$pkg."','".$searchlocationname."')";
				$exectmp1 = mysql_query($querytmp1) or die ("Error in Querytmp1".mysql_error());
				    ...
				    <tr>
                     ...
                     <td align="left" valign="top"  bgcolor="#E0E0E0" class="bodytext3"><span class="bodytext32">Sample Type</span></td>
					  ...
					  <tr>
                        ...
                        <?php /*?><!--<td align="left" valign="top"  bgcolor="#E0E0E0"><select id="taxanum" name="taxanum">
