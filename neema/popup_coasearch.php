...
if (isset($_REQUEST["id"])) { $idt = $_REQUEST["id"]; } else { $idt = ""; }
...
function escapekeypressed(e)
{
	...
	{ 
		...
		if (varSubString == "serialnumber") //focus is already on serial number.
		{
			...
			if (varSerialNumber >= 1)
			{
				...
				document.getElementById("customeraddress"+varUpdateRow).style.backgroundColor="#FFFFFF";
				document.getElementById("customergroup"+varUpdateRow).style.backgroundColor="#FFFFFF";
				...
				document.getElementById("customeraddress"+varUpdateRow).style.backgroundColor="#99FF00";
				document.getElementById("customergroup"+varUpdateRow).style.backgroundColor="#99FF00";
					//alert (document.activeElement.name);
		...
		else
		{
			...
			document.getElementById("customeraddress1").style.backgroundColor="#99FF00";
			document.getElementById("customergroup1").style.backgroundColor="#99FF00";
	...
	if(key == '13') //up arrow key press
	{
		...
		if (!isNaN(varActiveElementNumber)) //to prevent losing focus to submit2.
		{
			...
			var varTypeCatch =  document.getElementById("customeraddress"+varActiveElementNumber).value;
			varTypeCatch = varTypeCatch.toUpperCase();
			//alert (varCustomerNameCatch);
			var varGroup1 =  document.getElementById("customergroup"+varActiveElementNumber).value;
			//alert (varCustomerNameCatch);
			window.opener.document.getElementById("paynowlabcoa"+idt).value = varCoaCatch;
			window.opener.document.getElementById("paynowlabtype"+idt).value = varTypeCatch;
			window.opener.document.getElementById("paynowlabcode"+idt).value = varIdCatch;
			window.opener.document.getElementById("groupcode"+idt).value = varGroup1;
