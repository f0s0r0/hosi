...
if($assignmonth != '')
{
	$query99 = "select * from payroll_assign where status <> 'deleted' group by employeecode";
	$exec99 = mysql_query($query99) or die ("Error in Query99".mysql_error());
	while($res99 = mysql_fetch_array($exec99))
	{
		$employeesearch = $res99['employeecode'];
	
		$query12 = "select employeename, payrollstatus, employeecode from master_employee where employeecode = '$employeesearch'";
		...
		$query129 = "DELETE FROM `details_employeepayroll` WHERE employeecode = '$employeecode' and paymonth = '$assignmonth'";
		$exec129 = mysql_query($query129) or die ("Error in Query129".mysql_error());
		
		if($res12hold == 'Active')
		{		
			$query190 = "INSERT INTO `details_employeepayroll` (employeecode, employeename, paymonth, locationcode, locationname, ipaddress, username, updatedatetime)
				     VALUES ('$employeecode','$employeename','$assignmonth','$locationcode','$locationname','$ipaddress','$username','$updatedatetime')";
			...
			foreach($datatotal as $key => $value)
			{
				...
				$query191 = "UPDATE `details_employeepayroll` SET `$key` = '".array_sum($datatotal[$key])."' WHERE employeecode = '$employeecode' AND paymonth = '$assignmonth' AND locationcode = '$locationcode'";
