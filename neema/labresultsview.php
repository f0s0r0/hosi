     ...
     <tr>
	  <td colspan="5" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext365">
			...
			<td width="4%"  align="left" valign="center" 
                bgcolor="#ffffff" class="bodytext366"><div align="center"><strong>Reference Value</strong></div></td>
				  ...
				  <?php 
	  $query31="select * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and docnumber='$docnumber' group by itemname";
	  ...
	  { 
	   ...
	   $itemcode2=$res31['itemcode'];
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			$query521="select testcode as itemcode,testname as itemname,parametername as referencename,parametercode from pending_test_orders where testcode='$itemcode2' and patientcode = '$patientcode' and visitcode = '$visitcode' group by parametercode order by auto_number asc";
			$exec521=mysql_query($query521);
			$num521a=mysql_num_rows($exec521);
				if($num521a==0){
					 $query521="select * from master_labreference where itemname='$labname1' and itemcode='$itemcode2' and status <> 'deleted'";
				   $exec521=mysql_query($query521);
				  $num521=mysql_num_rows($exec521);
				}
			   while($res521=mysql_fetch_array($exec521))
			   {
				...
				$parametercode=$res521['parametercode'];
				   $referencename= $res521['referencename'];
			    

			    $query52="select * from master_labreference where itemname='$labname1' and itemcode='$itemcode2' and status <> 'deleted' and referencename='$referencename' ";
				...
				$labitemanum = $res52['auto_number'];
				  ...
				  $labreferencevalue1=$res52['referencevalue'];
		 					...
		 					$labreferencename1 = $labreferencename;

				 	if($num521a>0){
				 		$labreferencename1 = $parametercode;
				 	}
				   //echo $color = $res31['color'];
				 if($labreferencename != '')
				 {

				  $query87 = "select * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and docnumber='$docnumber' and referencename = '$labreferencename1'";
				  $exec87 = mysql_query($query87) or die(mysql_error());
				  $res87 = mysql_fetch_array($exec87);
				  $resultvalue = $res87['resultvalue'];
				 $color1 = $res87['color'];
				  }
				  else
				  {
				    $query87 = "select * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and docnumber='$docnumber'";
				  $exec87 = mysql_query($query87) or die(mysql_error());
				  $res87 = mysql_fetch_array($exec87);
				  $resultvalue = $res87['resultvalue'];
				  $color1 = $res87['color'];
				...
				//echo $labreferencerange1[0];
				 ...
				 $query49 = "select * from resultentry_lab where patientcode='$patientcode' and itemcode='$itemcode2' and referencename='$labreferencename1' and docnumber <> '$docnumber' order by auto_number desc limit 0,1";
				 ...
				 $pastresult = $res49['resultvalue'];
				  ...
				  <textarea readonly name="result[]" id="result<?php echo $sn; ?>" <?php if($color1 == 'orange') {  ?> style="background-color:orange;font-weight:bold;" <?php } else if($color1 == 'green'){ ?> style="background-color:green;font-weight:bold;"<?php } else if ($color1 == 'red'){ ?>style="background-color:red;font-weight:bold;"<?php }?>><?php echo $resultvalue;?></textarea></div></td>
      ...
      <tr>
              ...
              <td colspan="7" align="right" valign="top" >
             	  ...
             	  <a href="labresultsviewlist.php"><input name="Submit2223" type="button" value="Back" onClick="return acknowledgevalid()"  accesskey="b" class="button" style="border: 1px solid #001E6A"/></a>
               </td>
          </tr>
		  </table>
		  </td>
