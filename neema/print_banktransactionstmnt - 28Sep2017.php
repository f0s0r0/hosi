...
<table width="100%" border="1" align="left" cellpadding="4" cellspacing="0"  style="border-collapse: collapse">
				...
				?>
				<?php
				$qrybankstatements = "SELECT postdate,bankdate,remarks,docno,bankamount,notes FROM bank_record WHERE postdate < '$transactiondatefrom' AND bankcode = '$bankname' AND status='Posted'";
				  ...
				  <?php
				  $qrybankstatements = "SELECT postdate,bankdate,remarks,docno,bankamount,notes FROM bank_record WHERE postdate BETWEEN '$transactiondatefrom' AND  '$transactiondateto' AND bankcode = '$bankname' AND status='Posted'";
					$excebankstatements = mysql_query($qrybankstatements) or die("Error in qrybankstatements".mysql_error());
