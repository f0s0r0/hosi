...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 3 && $forloopcount != 5  && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			if($forloopcount == 4)
			{
			echo $suppliername = $splitdelimiter[3];
			
			echo $suppliername = strtoupper($suppliername);
			echo $suppliercode = $splitdelimiter[6];
			echo $suppliercode = substr($suppliercode,1,7);
			echo $suppliercode = strtoupper($suppliercode);
			...
			$rate = $splitdelimiter[3];
			$rate = strtoupper($rate);
			$amount = $splitdelimiter[4];
			...
			if ($itemname != '')
			{
			$query2 = "insert into master_nmrfq (itemname,suppliername,suppliercode,rate,amount,username,companyanum,ipaddress,recorddate,quantity) values ('$itemname','$suppliername','$suppliercode','$rate','$amount','$username','$companyanum','$ipaddress','$currentdate','$quantity')";
...
header("location:uploadnmrtq.php?st=success&&upload=success");
}
?>
<style type="text/css">
<!--
