...
//new query
/*$query2="SELECT MAX(CAST(SUBSTR(billnumber, LOCATE('-', billnumber)+1) AS UNSIGNED)) as billnumber FROM `purchase_details` WHERE typeofpurchase='Process' order by auto_number desc";
...
echo	$billnumbercode = 'GRN-' .$maxanum.'-'.$locationprefix;
	$openingbalance = '0.00';
	//echo $companycode;
}*/
//exit;
...
$itemstatus = '';
			...
			{		
				...
				if($billnumbercodekey==0)
				{
 $query2="SELECT MAX(CAST(SUBSTR(entrydocno, LOCATE('-', entrydocno)+1) AS UNSIGNED)) as billnumber FROM transaction_stock WHERE description='Purchase' order by auto_number desc";
$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
$res2 = mysql_fetch_array($exec2);
$billnumber = $res2["billnumber"];
$billdigit=strlen($billnumber);
...
else
{
	...
	//echo $companycode;
			...
			manufactureranum, manufacturername,typeofpurchase,suppliername,suppliercode,ponumber,supplierbillnumber,costprice,locationcode,store,coa,categoryname,fifo_code) 
			values ('$billautonumber', '$companyanum',";
...
else
{
	...
	//echo $companycode;
				...
				$billnumbercode=$billnumbercode;*/
			$query4 .=" '$billnumbercode', '$itemanum', '$itemcode', '$itemname', '$itemdescription', '$rate', '$quantity', 
			...
			'$packageanum', '$packagename', '$quantityperpackage', '$allpackagetotalquantity', 
			'$manufactureranum', '$manufacturername','Process','$suppliername','$suppliercode','$ponumber','$supplierbillno','$costprice','$locationcode','$store1','$coa','$categoryname','$fifo_code')";
		...
		$billnumbercodekey=$billnumbercodekey+1;
		...
		/*if($billnumbercodekey1==0)
				{
		$query2="SELECT MAX(CAST(SUBSTR(billnumber, LOCATE('-', billnumber)+1) AS UNSIGNED)) as billnumber FROM master_transactionpharmacy WHERE transactiontype='PURCHASE' order by auto_number desc";
$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
$res2 = mysql_fetch_array($exec2);
$billnumber = $res2["billnumber"];
$billdigit=strlen($billnumber);
...
else
{
	...
	//echo $companycode;
		...
		$query9 .= "	'$billnumbercode',  '$billautonumber', '$ipaddress', '$updatedate', '$companyanum', '$companyname', '$transactionmodule1','$suppliercode')";
...
else
{
	...
	//echo $companycode;
		...
		$query3 = "insert into master_purchase (companyanum, billnumber, billdate, suppliercode, suppliername,totalamount,totalquantity,ipaddress,supplierbillnumber,typeofpurchase)values('$companyanum',";
		
		/*if($billnumbercodekey1==0)
				{
		$query2="SELECT MAX(CAST(SUBSTR(billnumber, LOCATE('-', billnumber)+1) AS UNSIGNED)) as billnumber FROM master_purchase WHERE typeofpurchase='Process' order by auto_number desc";
$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
$res2 = mysql_fetch_array($exec2);
$billnumber = $res2["billnumber"];
$billdigit=strlen($billnumber);
...
else
{
	...
	//echo $companycode;
		...
		$query3 .= " '$billnumbercode','$updatedatetime','$suppliercode', '$suppliername','$amount','$quantity','$ipaddress','$supplierbillno','Process')";
...
$('#po').autocomplete({
	...
	source:'autocompletebuild_purchaseorder.php', 
	//alert(source);
	minLength:2,
	delay: 0,
	html: true, 
		select: function(event,ui){
		var po = ui.item.value;
		$('#po').val(po);
		$('#cbform1').submit();	
...
function checkrow(totalcount)
{
	var grandtotaladjchk = 0;
	for(i=1;i<=totalcount;i++)
	{
		var selectitem = document.getElementById("selectitem"+i).checked;
		//alert(selectitem);
		if(selectitem == true)
		{
			...
			grandtotaladjchk=parseFloat(grandtotaladjchk)+parseFloat(totaladjamount);
		}
	}
	document.getElementById("totalpurchaseamount").value = grandtotaladjchk.toFixed(2);
}
...
if(priceperpack2 == '') { priceperpack2 = 0; }
if(receivedqty2 != '')
{
...
if(discountpercent3 =='') { discountpercent3 = 0; }
...
var CostTprice = parseFloat(priceperpack3) / parseFloat(packsize3);

var costtax = parseFloat(CostTprice)

document.getElementById("costprice"+varserialnumber3+"").value = costtax.toFixed(2);
...
var finalamount3 = finaltaxamount3;
var tot1=parseFloat(receivedqty3) * parseFloat(packvalue3);
var costprice1 = priceperpack3;

var CostTprice = parseFloat(priceperpack3) / parseFloat(packsize3);
//alert(CostTprice+' cost');
//alert(finaltaxamount35+' tax');
var finaltaxamount25 = parseFloat(CostTprice) * parseFloat(tax);
var finaltaxamount35 = parseFloat(finaltaxamount25)/100;
//alert(finaltaxamount35);
var costtax = parseFloat(CostTprice) + parseFloat(finaltaxamount35);
var costtax = parseFloat(costtax);
//var costtax34 = parseFloat(costtax) / parseFloat(packsize3);
//var costtax34 = parseFloat(costtax34);
document.getElementById("costprice"+varserialnumber3+"").value = costtax.toFixed(2);
var salepricemarkup = (parseFloat(costtax) * parseFloat(spmarkup1))/100;
var saleprice = parseFloat(salepricemarkup) + parseFloat(costtax); 
var saleprice = parseFloat(saleprice);
//var saleprice = parseFloat(costprice1) + parseFloat(salepricemarkup);
document.getElementById("saleprice"+varserialnumber3+"").value = saleprice.toFixed(2);
}

var finalamount3 = parseFloat(finalamount3);
...
for(i=1;i<=totalcount1;i++)
{
	...
	if(selectitem == true)
	{
		...
		grandtotaladjamt1=parseFloat(grandtotaladjamt1)+parseFloat(totaladjamount);
	}
...
-->
  <tr>
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td>
		 <form name="cbform1" id="cbform1" method="post" action="goodsreceivednote.php"> 
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
		 ...
		 <input type="hidden" name="supplierbillno1" id="supplierbillno1">
			...
			//$packagequantity = round($packagequantity);
			 ...
			 $query444 = "select * from purchase_details where itemcode='$itemcode' and ponumber='$billnum'";
...
<?php $sno = $sno + 1; ?>
  <tr>
  		<td class="bodytext31" valign="center"  align="left"><input type="checkbox" class="selectitem" name="selectitem[]" id="selectitem<?php echo $sno ; ?>" value="<?php echo $itemcode; ?>" onClick="return checkrow('<?php echo $number; ?>')"></td>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="batch[]" id="batch<?php echo $sno; ?>" size="6" class="bodytext21" autocomplete="off"></div></td>
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="expirydate[]" id="expirydate<?php echo $sno; ?>" size="6" autocomplete="off"></div></td>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="free[]" id="free<?php echo $sno; ?>" size="6" onKeyUp="return totalcalc1('<?php echo $sno; ?>');" class="bodytext21" autocomplete="off"></div></td>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="tax[]" id="tax<?php echo $sno; ?>" size="6" onKeyUp="return totalamountdisc('<?php echo $sno; ?>','<?php echo $number; ?>');" class="bodytext21" autocomplete="off"></div></td>
