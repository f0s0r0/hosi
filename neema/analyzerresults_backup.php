...
$docnumber=$billnumbercode;
		 ...
		 $qrylabs = "select * from pending_test_orders where sample_id = '$docno' and visitcode = '$visitcode' and result <> '' group by testcode";
		...
		$orderedbyname = $res24['employeename'];
			$qryresult = "select result,date(resultdatetime) as recorddate, time(resultdatetime) as recordtime,parametername,referenceunit,referencerange from pending_test_orders as a join master_labreference as  b  on( a.testcode = b.itemcode and a.parametername = b.referencename) where a.result<>'' and b.status <> 'deleted'";
		...
		$refname = $resresults['parametername'];
   ...
   $query76 = "update samplecollection_lab set resultentry='$status1' where itemcode='$labcode' and patientvisitcode='$visitcode' and sampleid='$sampleid'";
 ...
 $query29=mysql_query("update consultation_lab set resultentry='$status1',resultdoc='$docnumber' where labitemcode='$labcode' and patientvisitcode='$visitcode' and docnumber='$sampleid'") or die(mysql_error());
   ...
   $query44 = "update consultation_lab set publishstatus='completed',publishdatetime='".date('Y-m-d h:i:s')."' where resultdoc='$docnumber'";
		...
		$query43 = "update pending_test_orders set publishstatus='completed',publishdatetime='".date('Y-m-d h:i:s')."' where sample_id='$docno' and testcode='$labcode' and visitcode='$visitcode'";
		$exec43 = mysql_query($query43) or die(mysql_error());
