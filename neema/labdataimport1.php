...
if($dellocation!='')
{
	 $query="delete from master_lab where location='$dellocation'"; 
	$exec=mysql_query($query);
	header ("location:labdataimport.php?upload=deleted");
}
...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$itemname_abbreviation = 'LABORATORY';
			...
			$sampletype = $splitdelimiter[7];
			$sampletype = strtoupper($sampletype);
			$taxname = '';
			$externallab = $splitdelimiter[6];
			$externallab = strtolower($externallab); 
			$ipmarkup = $splitdelimiter[4];
			...
			{
				$query89 = "select * from master_lab where itemcode = '$itemcode' and location = '$location'";
				$exec89 = mysql_query($query89) or die ("Error in Query89".mysql_error());
				$row89 = mysql_num_rows($exec89);
				if($row89 == 0)
				{	  
					$query2 = "insert into master_lab(itemcode,itemname,categoryname,sampletype,itemname_abbreviation,referencename,referenceunit,pkg,rateperunit,rate2,externallab,ipmarkup,location,locationname,updatetime,ipaddress) 
					values ('$itemcode','$itemname','$categoryname','$sampletype','$itemname_abbreviation','$referencename','$referenceunit','$pkg','$rateperunit','$rate2','$externallab','$ipmarkup','$location','$locationname','$updatedatetime','$ipaddress')  ";  
	...
	header('location:labdataimport.php?upload=success');
}
?>
<style type="text/css">
<!--
