...
<tr>
<td colspan="12" align="center" valign="middle"><strong>OP Paylater Refund Reconcile</strong></td>
</tr>
...
<?php 
$query8 = "SELECT visitcode,docno as billnumber,transactionamount,transactiondate,accountname,accountnameid FROM `master_transactionpaylater` WHERE `docno` LIKE 'Cr.N%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'paylatercredit'  order by auto_number";
...
$i = 0;
$drresult = array();
$querydr1bnk = "SELECT SUM(`labitemrate`) as income FROM `refund_paylaterlab` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`radiologyitemrate`) as income FROM `refund_paylaterradiology` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`amount`) as income FROM `refund_paylaterservices` WHERE billnumber = '$docno'";
       ...
       <tr>
        <td valign="top"><table width="600" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			{	
				...
				$query2 = "select accountssub,auto_number,tbinclude,tbledgerview from master_accountssub where auto_number IN ('15') AND accountsmain = '$parentid' and recordstatus <> 'deleted' order by accountssub";
	...
	if($parentid != '')
	{
		if($parentid == '15')
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(transactionamount) as paylatercr FROM `master_transactionpaylater` WHERE accountnameid = '$id' AND `docno` LIKE 'Cr.N%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'paylatercredit'";
				$execdr1 = mysql_query($querydr1bnk) or die ("Error in querydr1bnk".mysql_error());
				while($resdr1 = mysql_fetch_array($execdr1))
				{
				$i = $i+1;
				$drresult[$i] = $resdr1['paylatercr'];
				}
		...
		else if($parentid == '1')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT SUM(`labitemrate`) as income FROM `refund_paylaterlab` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`radiologyitemrate`) as income FROM `refund_paylaterradiology` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`amount`) as income FROM `refund_paylaterservices` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'";
