...
$updatedatetime = date('Y-m-d H:i:s');
	...
	if ($res2 == 0)
	{
		...
		if($pairvar!="")
		{
		$query61 = "select * from consultation_radiology where patientcode='$patientcode' and patientvisitcode='$visitcode' and consultationid='$consultationid' and consultationtime = '$timestamp'";
		...
		{	
			...
			{
		        //echo '<br>'. 
				$query65 = "select * from master_consultationpharm where patientcode='$patientcode' and patientvisitcode='$visitcode' and consultation_id='$consultationid' and consultationtime = '$timestamp'";
		$exec65 = mysql_query($query65) or die(mysql_error());
		$num65 = mysql_num_rows($exec65);
		if($num65 == 0)
		{
		        $query2 = "insert into master_consultationpharm(consultation_id,patientcode,patientauto_number,patientname,patientvisitauto_number,patientvisitcode,medicinename,dose,frequencyauto_number,frequencycode,frequencynumber,days,quantity,instructions,rate,amount,recordstatus,recorddate,ipaddress,consultingdoctor,billtype,accountname,paymentstatus,medicinecode,refno,pharmacybill,medicineissue,consultationtime) 
				values('$consultationid','$patientcode','$patientauto_number','$patientfullname','$patientvisit','$visitcode','$medicinename','$dose','$frequencyautonumber','$frequencycode','$frequencynumber','$days','$quantity','$instructions','$rate','$amount','completed','$currentdate','$ipaddress','$consultingdoctor','$billingtype','$accountname','pending','$medicinecode','$pharefcode','$status','pending','$timestamp')";
				$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
				
				$query29 = "insert into master_consultationpharmissue(consultation_id,patientcode,patientauto_number,patientname,patientvisitauto_number,patientvisitcode,medicinename,dose,frequencyauto_number,frequencycode,frequencynumber,days,quantity,instructions,rate,amount,recordstatus,recorddate,ipaddress,consultingdoctor,billtype,accountname,paymentstatus,medicinecode,refno,prescribed_quantity) 
				...
				$newquery=mysql_query("update master_triage set consultation='completed',urgentstatus='0' where visitcode='$visitcode'") or die(mysql_error());
				}
				
			}
		
		}
		$query63 = "select * from master_consultation where patientcode='$patientcode' and patientvisitcode='$visitcode' and consultation_id='$consultationid' and consultationtime = '$timestamp'";
...
$maxanum = $labrefcode;
	...
	$categorylabquery1=mysql_query("insert into consultation_lab(consultationid,patientcode,patientname,patientvisitcode,labitemname,labitemrate,billtype,accountname,consultationdate,paymentstatus,refno,labsamplecoll,resultentry,labrefund)values('$consultationid','$patientcode','$patientfullname','$visitcode','$categorylabname','$categorylabrate','$billingtype','$accountname','$currentdate','$status','$labrefcode','pending','pending','norefund')") or die(mysql_error());
		...
		if($labname!="")
		{
		$query63 = "select * from consultation_lab where patientcode='$patientcode' and patientvisitcode='$visitcode' and consultationid='$consultationid' and consultationtime = '$timestamp'";
		$exec63 = mysql_query($query63) or die(mysql_error());
		$num63 = mysql_num_rows($exec63);
		if($num63 == 0)
		{
		...
		if($pairvar2!="")
		{
		$query68 = "select * from consultation_referal where patientcode='$patientcode' and patientvisitcode='$visitcode' and consultationid='$consultationid' and consultationtime = '$timestamp'";
		...
		header ("location:revisitpatientlist.php?patientcode=$patientcode&&st=success");
		exit;
		}
	}
	else
...
<table width="103%" border="0" cellspacing="0" cellpadding="2">
  ...
  <tr>
    ...
    <td width="97%" valign="top">
     <form name="form1" id="form1" method="post" action="revisitconsultationform.php" onKeyDown="return disableEnterKey(event)" onSubmit="return process1();">
				...
				<td colspan="6" align="left" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo ''; } else { echo '#AAFF00'; } ?>" class="bodytext3"><?php echo $errmsg;?>&nbsp;</td>
				 ...
				 <tr bgcolor="">
				   ...
				   <td align="left" valign="middle"  bgcolor="" class="bodytext3"><label><?php echo $height;?>
					 ...
					 <input type="hidden" name="billtype" id="billtypes" value="<?php echo $patienttype; ?>">
                   </label></td>
				 ...
				 <tr bgcolor="">
				  ...
				  <td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">&nbsp;</td>
				   ...
				   <tr>
				   <td align="left" valign="middle"  bgcolor="" class="bodytext32"><span class="bodytext32"><strong>Drug Allergy</strong></span></td>
				  ...
				  <td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">&nbsp;</td>
				   ...
				   <td align="left" valign="middle"  bgcolor=""><span class="bodytext32"><?php echo $complanits; ?>
