         ...
         for ($p=1;$p<=20;$p++)
		  {	
			...
			$amount = $_REQUEST['amount'.$p];
				...
				{
					//echo '<br>'. 
					$query2 = "insert into master_consultationpharm(patientcode,patientname,patientvisitcode,medicinename,dose,frequencyauto_number,frequencycode,frequencynumber,days,quantity,instructions,rate,amount,recordstatus,recorddate,ipaddress,paymentstatus,medicinecode,refno,pharmacybill,medicineissue,store,location,billtype,accountname,locationcode) 
					values('$patientcode','$patientname','$visitcode','$medicinename','$dose','$frequencyautonumber','$frequencycode','$frequencynumber','$days','$quantity','$instructions','$rate','$amount','completed','$currentdate','$ipaddress','$status','$medicinecode','$pharefcode','$status','pending','$store','$location','$billingtype','$accountname','$locationcode')";
					$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
					
					$query29 = "insert into master_consultationpharmissue(patientcode,patientname,patientvisitcode,medicinename,dose,frequencyauto_number,frequencycode,frequencynumber,days,quantity,instructions,rate,amount,recordstatus,recorddate,ipaddress,paymentstatus,medicinecode,refno,prescribed_quantity,store,location,billtype,accountname,locationcode) 
					values('$patientcode','$patientname','$visitcode','$medicinename','$dose','$frequencyautonumber','$frequencycode','$frequencynumber','$days','$quantity','$instructions','$rate','$amount','completed','$currentdate','$ipaddress','$status','$medicinecode','$pharefcode','$quantity','$store','$location','$billingtype','$accountname','$locationcode')";
...
$maxanum = $labrefcode;
	...
	if($categorylabname != "")
	{
	$categorylabquery1=mysql_query("insert into consultation_lab(patientcode,patientname,patientvisitcode,labitemname,labitemrate,consultationdate,paymentstatus,labsamplecoll,resultentry,labrefund,refno,locationcode)values('walkin','$patientfullname','walkinvis','$categorylabname','$categorylabrate','$currentdate','paid','pending','pending','norefund','$labrefcode','$locationcode')") or die(mysql_error());
	}	
	}
		...
		if($num63 == 0)
		{
			$labquery1=mysql_query("insert into consultation_lab(patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,consultationdate,paymentstatus,labsamplecoll,resultentry,labrefund,refno,billtype,accountname,username,freestatus,locationcode)values('$patientcode','$patientname','$visitcode','$labcode','$labname','$labrate','$currentdate','$mchstatus','pending','pending','norefund','$labrefcode','$billingtype','$accountname','$username','$labfree','$locationcode')") or die(mysql_error());
			}
			}
		}
		...
		if($num61 == 0)
		{
		$radiologyquery1=mysql_query("insert into consultation_radiology(patientcode,patientname,patientvisitcode,radiologyitemcode,radiologyitemname,radiologyitemrate,billtype,accountname,consultationdate,paymentstatus,refno,resultentry,username,locationcode)values('$patientcode','$patientname','$visitcode','$radiologycode','$pairvar','$pairvar1','$billingtype','$accountname','$currentdate','$status','$radrefcode','pending','$username','$locationcode')") or die(mysql_error());
		}
		}
		}
		...
		$servicesquery1=mysql_query("insert into consultation_services(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,consultationdate,paymentstatus,process,refno,billtype,accountname,username,locationcode,serviceqty,amount)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$currentdate','$status','pending','$serrefcode','$billingtype','$accountname','$username','$locationcode','$serviceqty','$seramount')") or die(mysql_error());
			}
		}
...
<form name="form1" id="frmsales" method="post" action="mchtriagebilling.php" onKeyDown="return disableEnterKey(event)" onSubmit="return processcheck()">
			 ...
			 <tr>
				  ...
				  <tr id="labid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
					 ...
					 <div id="insertrow1">					 </div></tr>
					  ...
					  <td><label>
                       <input type="button" name="Add1" id="Add1" value="Add" onClick="return insertitem20()" class="button" style="border: 1px solid #001E6A">
                       </label></td>
					   </tr>
					    </table>	  </td> 
				  </tr>
				...
				<tr id="serid">
				   ...
				   <td width="30"><input name="services[]" type="text" id="services" size="69" autocomplete="off"></td>
					...
					<td width="30"><input name="serviceamount[]" type="text" id="serviceamount" readonly size="8">
<input type="hidden" id="baseunit">
<input type="hidden" id="incrqty">
<input type="hidden" id="incrrate">
<input type="hidden" id="slab">
<input type="hidden" id="pkg2">
<input type="hidden" id="packcharge">
<select id="servicesfree" style="display:none"></select>
</td>
					   <td><label>
