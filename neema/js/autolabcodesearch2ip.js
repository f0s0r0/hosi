...
function ajaxprocessACCS25()
{
	...
	var url = "";
	var url="autolabcodesearch2ip.php?RandomKey="+Math.random()+"&&labsearch="+labsearch+"&&patienttypesearch="+varpatienttype+"&&varpaymenttype="+varpaymenttype+"&&locationcode="+locationcode+"&&subtype="+subtype;
    //alert(url);
...
{ 
	...
	//{
		...
		//alert(varlabRate);
		//alert(ans);
	//	var percent=(parseFloat(varlabRate)*parseFloat(varlabmarkup/100));
		//alert(percent);
	//	var rateorginal=percent+(parseFloat(varlabRate));
		//alert(rateorginal);
		...
		//alert (varlabRate);
		document.getElementById("pkg").value = varpkg; //alert(varpkg);
		
		var Packapp = document.getElementById("packcharge").value;
		
		document.getElementById("labfree").options.length = null;
		
		if(Packapp == 1)
		{
			if(varpkg=='yes')
			{
				var x = document.getElementById("labfree");
