...
function funccategorylabsearch5()
{
	//alert("Meow...");
	if(document.getElementById("categorylabcode").value!="")
	{
		var varcategorylabsearch = document.getElementById("categorylabcode").value;
		//alert (varmedicinesearch);
		var varcategorylabsearchLen = varcategorylabsearch.length;
		//alert (varmedicinesearchLen);
		if (varcategorylabsearchLen >= 1)
		{
			ajaxprocessACCS35();		
		}
		//alert("Meow...");
...
function ajaxprocessACCS35()
{
	...
	if (xmlHttp==null)
	{
  	...
  	var categorylabsearch = document.getElementById("categorylabcode").value;
	//alert(medicinesearch);
	var url = "";
	var url="autocategorylabcodesearch2.php?RandomKey="+Math.random()+"&&categorylabsearch="+categorylabsearch;
    //alert(url);

	xmlHttp.onreadystatechange=stateChangedACCS35 
	xmlHttp.open("GET",url,true)
	xmlHttp.send(null)
} 

function stateChangedACCS35() 
{ 
	...
	//{
		...
		var varcategorylabCode = varNewLineValue[0];
		//alert (varMedicineCode);
		document.getElementById("categorylabcode").value = varcategorylabCode;

		var varcategorylabName = varNewLineValue[1];
		//alert (varMedicineName);
		document.getElementById("categorylab").value = varcategorylabName;

		var varcategorylabRate = varNewLineValue[2];
		//alert (varMedicineName);
		document.getElementById("categoryrate5").value = varcategorylabRate;
	...
	//alert(varlabRate);
		...
		document.getElementById("categorylab").value = Varcategorylabname1;
