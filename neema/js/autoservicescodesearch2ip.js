...
function ajaxprocessACCS27()
{
	...
	var url = "";
	var url="autoservicescodesearch2ip.php?RandomKey="+Math.random()+"&&servicessearch="+servicessearch+"&&patienttypesearch="+varpatienttype2+"&&varpaymenttype="+varpaymenttype+"&&locationcode="+locationcode+"&&subtype="+subtype;
    //alert(url);
...
{ 
	...
	//{
		...
		var varlabmarkup = varNewLineValue[8];
		//var varlabmarkup=23;
		//var ans=parseFloat(varlabmarkup/100);
		//alert(varservicesRate);
		//alert(ans);
		var percent=(parseFloat(varservicesRate)*parseFloat(varlabmarkup/100));
		//alert(percent);
		var rateorginal=percent+(parseFloat(varservicesRate));
		//alert(rateorginal);
		document.getElementById("rate3").value = rateorginal;
		...
		document.getElementById("servicesfree").options.length = null;
