...
if (xmlHttp==null)
  ...
  return
	...
	var url = "";
	var url="autocompletebatchnumberippharmacyissueip.php?RandomKey="+Math.random()+"&&itemcode="+varItemCode+"&&locationcode="+locationcode;
	//alert(url);
		...
		for (i=0;i<9;i++)
		{
				...
				if (brokenstring[i] != null)
				{
					...
					*/
					//Ref: http://viralpatel.net/blogs/dynamic-combobox-listbox-drop-down-using-javascript/
					var newOption = document.createElement('option');
					 brokebranch=brokenstring[i].split("(");
					 newOption.text =brokebranch[0];
