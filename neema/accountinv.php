            ...
            <?php
				...
				{  
					...
					if($id == '03-1001-01')
					{
						$i = 0;
						$drresult = array();
						$querydr1inv = "SELECT SUM(`totalamount`) as stock FROM `purchase_details` WHERE `ponumber` NOT LIKE '%MLPO%' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' AND `suppliername` <> 'OPENINGSTOCK' AND `expensecode` = '' AND `recordstatus` <> 'deleted'
						...
						$querycr1inv = "SELECT SUM(`totalamount`) as stockcredit FROM `purchasereturn_details` WHERE `entrydate` BETWEEN '$ADate1' AND '$ADate2'
										...
										UNION ALL SELECT SUM(a.amount) as stockcredit FROM `master_stock_transfer` AS a JOIN `master_medicine` AS b ON (a.itemcode = b.itemcode) WHERE a.typetransfer = 'Consumable' AND b.inventorytype <> 'non-medical' AND a.`entrydate` BETWEEN '$ADate1' AND '$ADate2'";
					...
					else if($id == '03-1002-01')
					{
						$i = 0;
						$drresult = array();
						$querydr1inv = "SELECT SUM(`totalamount`) as stock FROM `purchase_details` WHERE `ponumber` LIKE '%MLPO%' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' AND `suppliername` <> 'OPENINGSTOCK' AND `expensecode` = '' AND `recordstatus` <> 'deleted'
										UNION ALL SELECT SUM(debitamount) as stock FROM `master_journalentries` WHERE `ledgerid` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'";
						$execdr1 = mysql_query($querydr1inv) or die ("Error in querydr1inv".mysql_error());
						while($resdr1 = mysql_fetch_array($execdr1))
						{
					...
					<a href="detail_accountinv.php?id=<?= $id; ?>&&ADate1=<?= $ADate1; ?>&&ADate2=<?= $ADate2; ?>" target="_blank"><?php echo $colorloopcount; ?>. <?php echo $accountsmain2; ?></a></strong></td>
