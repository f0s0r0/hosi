        ...
        <td valign="top"><table width="919" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
            ...
            <?php
				...
				{  
					...
					$queryjr = "SELECT `debitamount` as td1, `entrydate` as td2, `docno` as td3, `narration` as td4, `ledgerid` as td5, `ledgername` as td6 FROM `master_journalentries` WHERE ledgerid = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'
								UNION ALL SELECT `creditamount` as td1, `entrydate` as td2, `docno` as td3, `narration` as td4, `ledgerid` as td5, `ledgername` as td6 FROM `master_journalentries` WHERE ledgerid = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'";
					$execjr = mysql_query($queryjr) or die ("Error in Queryjr".mysql_error());
					while($resjr = mysql_fetch_array($execjr))
					{
					$amount = $resjr['td1'];
					$entrydate = $resjr['td2'];
					$visitcode = $resjr['td3'];
					$patientname = $resjr['td4'];
					$itemcode = $resjr['td5'];
					$itemname = $resjr['td6'];
