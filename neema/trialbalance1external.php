	...
	if($parentid != '' && $tbinclude != '')
	{
		...
		{  
			...
			if($id == '01-1006')
			{
				$j = 0;
				$crresult = array();  
				$querycr1in = "SELECT SUM(`labitemrate`) as income FROM `billing_externallab` WHERE `billdate` BETWEEN '$ADate1' AND '$ADate2'
			...
			{	
				$i = 0;
				$drresult = array();
				$querydr1bnk = "SELECT SUM(`cashamount`) as bankamount FROM `master_transactionexternal` WHERE (`cashcode` = '$id') AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
