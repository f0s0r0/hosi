...
function ledgervalue($parentid,$ADate1,$ADate2,$tbinclude)
{
	...
	{
		$query8 = mysql_query("select accountsmain from master_accountssub where auto_number = '$parentid'");
		$res8 = mysql_fetch_array($query8);
		//echo $res8['accountsmain'];
		$query81 = mysql_query("select section from master_accountsmain where auto_number = '".$res8['accountsmain']."'");
		$res81 = mysql_fetch_array($query81);
		//echo $res81['section'];
		...
		{  
			...
			if($res81['section'] == 'A' || $res81['section'] == 'E')
			{
				$i = 0;
				$result = array();
				$querydr1 = "SELECT SUM(debitamount) as stockpurchase FROM `master_journalentries` WHERE `ledgerid` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'
				UNION ALL SELECT SUM(-1*creditamount) as stockpurchase FROM `master_journalentries` WHERE `ledgerid` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'";
				...
				$accountbank = array_sum($result);
			}
			else
			{
				$j = 0;
				$resultcr = array();
				$querycr1 = "SELECT SUM(creditamount) as stockreturn FROM `master_journalentries` WHERE `ledgerid` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'
				UNION ALL SELECT SUM(-1*debitamount) as stockreturn FROM `master_journalentries` WHERE `ledgerid` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'";
				...
				$accountbank = array_sum($resultcr);
			}
