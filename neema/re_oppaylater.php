...
<tr>
<td colspan="12" align="center" valign="middle"><strong>OP Paylater Reconcile</strong></td>
</tr>
...
<td align="right"><strong>Dept Referal (Cr)</strong></td>
<td align="right"><strong>Total</strong></td>
<td align="right"><strong>Diff</strong></td>
<td align="center"><strong>Status</strong></td>
</tr>
<?php 
$query8 = "SELECT visitcode,billnumber,transactionamount,transactiondate,accountname,accountnameid FROM `master_transactionpaylater` WHERE `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'finalize' AND `billnumber` LIKE 'CB%' order by auto_number";
...
$i = 0;
$drresult = array();
$querydr1bnk = "SELECT SUM(`totalamount`) as income FROM `billing_paylaterconsultation` WHERE billno = '$docno'
UNION ALL SELECT SUM(`labitemrate`) as income FROM `billing_paylaterlab` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`amount`) as income FROM `billing_paylaterpharmacy` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`radiologyitemrate`) as income FROM `billing_paylaterradiology` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`amount`) as income FROM `billing_paylaterservices` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`referalrate`) as income FROM `billing_paylaterreferal` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`subtotal`) as income FROM `billing_referal` WHERE billnumber = '$docno'";
       ...
       <tr>
        <td valign="top"><table width="600" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			{	
				...
				$query2 = "select accountssub,auto_number,tbinclude,tbledgerview from master_accountssub where auto_number IN ('18','19','20','21','22','17') AND accountsmain = '$parentid' and recordstatus <> 'deleted' order by accountssub";
...
function ledgervalue($parentid,$ADate1,$ADate2,$tbinclude)
{
	...
	if($parentid != '')
	{
		if($parentid == '21' || $parentid == '22' || $parentid == '17' || $parentid == '18' || $parentid == '19' || $parentid == '20')
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(`transactionamount`) as paylater FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'finalize' AND `billnumber` LIKE '%CB%'";
				$execdr1 = mysql_query($querydr1bnk) or die ("Error in querydr1bnk".mysql_error());
				while($resdr1 = mysql_fetch_array($execdr1))
				{
				$i = $i+1;
		...
		else if($parentid == '1')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT SUM(`totalamount`) as income FROM `billing_paylaterconsultation` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							...
							UNION ALL SELECT SUM(`subtotal`) as income FROM `billing_referal` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'";
