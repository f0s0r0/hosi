...
<td align="right"><strong>Mor Service (Cr)</strong></td>
...
<?php 
$query8 = "SELECT visitcode,billnumber,transactionamount,cashamount,chequeamount,cardamount,onlineamount,creditamount,transactiondate,bankcode,cashcode,mpesacode FROM `master_transactionexternal` WHERE `transactiondate` BETWEEN '$ADate1' AND '$ADate2' order by auto_number";
...
UNION ALL SELECT SUM(`servicesitemrate` * `quantity`) as income FROM `mortuaryexternal_services` WHERE billno = '$docno'";
	...
	if($parentid != '')
	{
		if($parentid == '18' || $parentid == '19' || $parentid == '20')
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(`creditamount`) as bankamount FROM `master_transactionexternal` WHERE `mpesacode` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
		...
		else if($parentid == '1')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT SUM(`labitemrate`) as income FROM `billing_externallab` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT ROUND(SUM(`amount`)) as income FROM `billing_externalpharmacy` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
