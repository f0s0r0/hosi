        ...
        <td valign="top"><table width="919" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
            ...
            <?php
				...
				{  
					...
					if($id == '07-8101-01')
					{
					$querydr1 = "SELECT (1*ABS(amount)) as deposit,recorddate as td2, docno as td3, patientcode as td4, patientname as td5 FROM `billing_ipnhif` WHERE visitcode IN (select visitcode FROM `master_transactionip` WHERE transactiondate BETWEEN '$ADate1' AND '$ADate2' group by billnumber)
								 UNION ALL SELECT SUM(-1*transactionamount) as deposit,transactiondate as td2, docno as td3, accountnameid as td4, accountname as td5 FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `docno` LIKE 'AR-%' AND `transactionstatus` = 'onaccount' AND `transactionmodule` = 'PAYMENT'";
					}
					...
					{
					$querydr1 = "SELECT (`rate`) as deposit,consultationdate as td2, docno as td3, patientcode as td4, patientname as td5 FROM `ip_discount` WHERE patientvisitcode IN (select visitcode FROM `master_transactionip` WHERE transactiondate BETWEEN '$ADate1' AND '$ADate2' group by billnumber)";
					}
					...
					?>
					<tr bgcolor="#CCC">
					<td colspan="7" align="right" class="bodytext3"><strong><?php echo number_format(abs($sumbalance),2); ?></strong></td>
					</tr>
</tbody>				
</table>
</td>
