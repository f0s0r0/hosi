...
//ends here
	...
	{// echo 'in';
		$query76 = "select * from master_visitentry where billtype='PAY LATER' and visitcode='$visitcode'";
		$exec76 = mysql_query($query76) or die(mysql_error());
		$num76 = mysql_num_rows($exec76);
		if($num76 == 0)
		{ //echo 'll';
		
		$querycumstock2 = "select cum_quantity from transaction_stock where cum_stockstatus='1' and itemcode='$itemcode' and locationcode='$getlocationcode' and storecode='$getstorecode'";
		...
		$query43="insert into pharmacysalesreturn_details(fifo_code,patientcode,visitcode,companyanum,billnumber,itemcode,itemname,rate,quantity,totalamount,username,ipaddress,entrydate,batchnumber,billstatus,location,pharmacycoa,categoryname,locationname,locationcode,store,docnumber,salseautonumber,comments)values('$fifo_code','$patientcode','$visitcode','$companyanum','$billnumber','$itemcode','$medicinename','$rate','$returnquantity','$amount','$username','$ipaddress','$dateonly','$batchnumber','pending','$location1','$pharmacycoa','$categoryname','".$getlocationname."','".$getlocationcode."','".$getstorecode."','".$docnumber."','".$salseautonumber."','$comments')"; 
		...
		$stockquery2="insert into transaction_stock (fifo_code,tablename,itemcode, itemname, transaction_date,transactionfunction,description,
			batchnumber, batch_quantity, 
			transaction_quantity, cum_quantity, entrydocno, docstatus, cum_stockstatus, batch_stockstatus,locationcode,locationname,storecode,storename,username,ipaddress,recorddate,recordtime,updatetime,patientcode,patientvisitcode,patientname)
			values ('$fifo_code','pharmacysalesreturn_details','$itemcode', '$medicinename', '$dateonly','1', 'Sales Return', 
			'$batchnumber', '$bat_quantity', '$returnquantity', 
			'$cum_quantity', '$billnumber', '','1','1', '$getlocationcode','','$getstorecode', '', '$username', '$ipaddress','$dateonly','$timeonly','$datetimeonly','$patientcode','$visitcode','$patientname')";
			
		$stockexecquery2=mysql_query($stockquery2) or die(mysql_error());
		
		$query43="insert into pharmacysalesreturn_details(patientcode,visitcode,companyanum,billnumber,itemcode,itemname,rate,quantity,totalamount,username,ipaddress,entrydate,batchnumber,billstatus,pharmacycoa,location,categoryname,locationname,locationcode,store,docnumber,salseautonumber,comments)values('$patientcode','$visitcode','$companyanum','$billnumber','$itemcode','$medicinename','$rate','$returnquantity','$amount','$username','$ipaddress','$dateonly','$batchnumber','completed','$pharmacycoa','$location1','$categoryname','".$getlocationname."','".$getlocationcode."','".$getstorecode."','".$docnumber."','".$salseautonumber."','$comments')";
		$exec43=mysql_query($query43) or die(mysql_error());
...
else { document.getElementById("submit123").disabled = true; }
}
...
/*echo $queryget = "select pd.locationname,pd.locationcode,ms.storecode FROM pharmacysales_details as pd LEFT JOIN master_store as ms ON pd.store = ms.store WHERE pd.patientcode = '".$patientcode."' AND pd.visitcode = '".$visitcode."'";*/
 $queryget = "select locationname,locationcode,store FROM pharmacysales_details WHERE patientcode = '".$patientcode."' AND visitcode = '".$visitcode."' and docnumber = '".$getdocnumber."'";
        ...
        <td><table width="99%" border="0" align="left" cellpadding="2" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
            <tbody>
              <tr bgcolor="#011E6A">
                    ...
                    <input name="billnumberprefix" id="billnumberprefix" value="<?php echo $billnumberprefix; ?>" type="hidden" style="border:none;"  size="5" /> 
                    <input type="hidden" name="patientcode" value="<?php echo $patientcode; ?>">
               <td bgcolor="#CCCCCC" class="bodytext3"><strong>Patient  </strong></td>
	  <td width="28%" align="left" valign="middle" bgcolor="#CCCCCC" class="bodytext3">
				<input type="hidden" name="customername" id="customer" value="<?php echo $Patientname; ?>" style="" size="40" autocomplete="off" readonly/><strong><?php echo $Patientname; ?></strong>
                  </td>
                ...
                <td  colspan="5" bgcolor="#CCCCCC" class="bodytext3">
                  ...
                  <input name="ADate" id="ADate" style="border:none;" value="<?php echo $dateonly; ?>"  size="8"  readonly="readonly" onKeyDown="return disableEnterKey()" />
			  ...
			  <tr>
                ...
                <td width="28%" align="left" valign="middle" class="bodytext3">
			<input type="hidden" name="visitcode" id="visitcode" value="<?php echo $visitcode; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" readonly/><strong><?php echo $visitcode; ?></strong>
                  </td>
				...
				<strong><?php echo $patientcode; ?></strong>
				<!--<textarea name="deliveryaddress" cols="25" rows="3" id="deliveryaddress" style="border: 1px solid #001E6A"><?php //echo $res41deliveryaddress; ?></textarea>--></td>
                 <td width="6%" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong>Location </strong></td>
                <td width="15%" colspan="3" align="left" valign="middle" class="bodytext3"><strong><?php echo $getlocationname; ?></strong>
				<input type="hidden" name="customercode" id="customercode" value="<?php echo $getlocationname; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" readonly/>		<input type="hidden" name="getlocationcode" value="<?php echo $getlocationcode; ?>">
			  ...
			  <td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><span class="style4"></span><strong>Age &amp; Gender </strong></td>
			    <td align="left" valign="middle" class="bodytext3"><strong><?php echo $patientage; ?></strong>
                ...
                <td colspan="3" align="left" valign="middle" class="bodytext3"><strong><?php echo $accountname; ?></strong>
				<input type="hidden" name="account" id="account" value="<?php echo $accountname; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" readonly/>
				...
				<input type="hidden" name="pharmacycoa" value="<?php echo $pharmacycoa; ?>"></td>
                 <td width="6%" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong>Store</strong></td>
                <td colspan="3" align="left" valign="middle" class="bodytext3"><strong><?php echo $getstorename; ?></strong>
				<input type="hidden" value="<?php echo $getstorename; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" readonly/>
				<input type="hidden" name="getstorecode" value="<?php echo $getstorecode; ?>"></td>
				
				  </tr>
				  <tr>
				      <td width="10%" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong>Return No </strong></td>
                <td colspan="3" align="left" valign="middle" class="bodytext3"><strong><?php echo $billnumbercode; ?></strong>
				<input type="hidden" name="billnumber" id="billnumber" value="<?php echo $billnumbercode; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" readonly/>
			</td>
				  </tr>
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$totalavaquantity=0;
			$totalavaquantity1=0;
			$query71 = "select * from pharmacysales_details where locationcode='".$locationcode."' and patientcode = '$patientcode' and visitcode = '$visitcode' and docnumber='$docnumber' ";
...
$nno=$nno+1;
	...
	//echo $nno;
			$query61 = "select * from pharmacysales_details where locationcode='".$locationcode."' and patientcode = '$patientcode' and visitcode = '$visitcode' and docnumber='$docnumber'";
...
//batch qty
 $querybatstock2 = "select batch_quantity from transaction_stock where batch_stockstatus='1' and itemcode='$itemcode' and locationcode='$locationcode' and fifo_code='$fifo_code' and storecode ='$getstorecode'";
...
?>
  ...
  <td class="bodytext31" valign="center"  align="left"><div align="center">
        ...
        <input type="hidden" name="fifo_code[]" value="<?php echo $fifo_code; ?>">
			...
			<input type="hidden" name="totalabatchqty[]" value="<?php echo $totalabatchqty;?>">
		...
		?>
			 ...
			 <tr>
              ...
              <td colspan="3" class="bodytext31" valign="center"  align="left" 
                bgcolor="#cccccc"><input type="hidden" name="comments" id="comments"/></td>
          ...
          <tbody>
            <tr>
              <td width="54%" align="right" valign="top" >
             	  ...
             	  <input name="Submit2223" id="submit123" type="submit" value="Save" onClick="return check();" accesskey="b" class="button" style="border: 1px solid #001E6A"/>
               </td>
