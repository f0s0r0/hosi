...
$locationcode12=isset($_REQUEST['locationcode'])?$_REQUEST['locationcode']:'';
...
foreach($_POST['billnum2'] as $key => $value)
		...
		if($acknow1==$billnum2)
		{
		$query84="update billing_ipnhif set acc_flag = '0',receivable_status='deallocated',transactionamount='0' where docno='$billnum2' and receivable_status <> 'deallocated'";
		$exec84=mysql_query($query84) or die(mysql_error());
		}
		}
		}
header("location:nhifreceivableentry.php?docno=$docnum&&locationcode=$locationcode1");
}
...
$locationcode13 = $_REQUEST['locationcode13'];
...
foreach($_POST['billnum'] as $key => $value)
		...
		$docnum1=$_POST['docno1'][$key];
		$accdocno1=$_POST['accdocno1'][$key];
		...
		{
			$transactionamount='0';
			$transdetail="select transactionamount from billing_ipnhif where acc_docno='$accdocno1' and docno='$docnum1'";
			$query=mysql_query($transdetail);
			$restran=mysql_fetch_array($query);
			$transactionamount=$restran['transactionamount'];
			
		 $adjamount=$adjamount+$transactionamount;
		
 	$query9912="update billing_ipnhif set acc_flag='$acc_flag',acc_docno='$accdocno1',receivable_status='$billstatus',receivable_user='$username',balanceamount='$balamount',transactionamount='$adjamount' where docno='$billnum' and visitcode='$visitcode' and locationcode='$locationcode13'"; 
	$exec9912=mysql_query($query9912) or die("Error in query9912".mysql_error());
	...
	//exit;
	header("location:nhifreceivableentry.php?docno=$accdocno1&&locationcode=$locationcode13");
}
  ...
  <tr>
    ...
    <td width="97%" valign="top"><table border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td width="709">
              ...
              <form name="cbform1" method="post" action="nhifreceivableentry.php">
	...
	<td class="bodytext31" valign="center"  align="left"><input type="hidden" name="bankname" value="<?php echo $bankname; ?>" size="6" class="bal"><?php echo $bankname; ?></td>
           ...
           <tr>
			  ...
			  $colorloopcount = 0;
			  $totamount = 0;
            $query2 = "select * from billing_ipnhif where acc_docno='$docno' and receivable_status in ('allocated','pending') and locationcode='$locationcode12'";
			 ...
			 // echo $num2;
              ...
              <input type="hidden" name="locationcode1" id="locationcode1" value="<?=$locationcde;?>">
	  ...
	  <tr>
	  <td>
	  <form action="nhifreceivableentry.php" method="post" name="form2">
	  <table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
		...
		$totalrefundedamount=0;
			...
			$num=0;
			$ssno = 0;
			$query2 = "select patientname, patientcode, visitcode, docno, recorddate, balanceamount,locationcode,amount,receivable_status from billing_ipnhif where  acc_flag <> '1' and receivable_status <>'allocated' and locationcode='$locationcode12'";
			$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
			...
			//	}
				...
				$receivable_status=$res2['receivable_status'];
				
				$amount=abs($res2['amount']);
				$billtotalamount = $res2['balanceamount'];
				$ssno = $ssno + 1;
			...
			$netpayment = number_format($netpayment, 2, '.', '');
			 $balanceamount = number_format($balanceamount, 2, '.', ''); 
			if($receivable_status <>'allocated' && $receivable_status <>'pending')
			{
            ...
            <tr <?php echo $colorcode; ?>>
			  ...
			  <td class="bodytext311" valign="center" bordercolor="#f3f3f3" align="left">
                            <input type="hidden" name="locationcode13" id="locationcode13" value="<?=$locationcde;?>">
			  ...
			  <input type="hidden" name="docno1[]" value="<?php echo $billnumber; ?>">
               <input type="hidden" name="accdocno1[]" value="<?php echo $docno; ?>">
			  <input type="hidden" name="paymentmode1" value="<?php echo $paymentmode; ?>" size="6" class="bal">
			  <input type="hidden" name="date1" value="<?php echo $date; ?>" size="6" class="bal">
			  <input type="hidden" name="number1" value="<?php echo $number; ?>" size="6" class="bal">
			  <input type="hidden" name="bankname1" value="<?php echo $bankname; ?>" size="6" class="bal">
