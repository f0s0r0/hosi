...
$query91 = "select * from master_triagebilling order by auto_number desc limit 0, 1";
	 $exec91 = mysql_query($query91) or die ("Error in Query1".mysql_error());
	 $rowcount91 = mysql_num_rows($exec91);
	if ($rowcount91 == 0)
	{
		$consultationcode = 'TRG001';
	}
	else
	{
		$res91 = mysql_fetch_array($exec91);
		 $consultationcode = $res91['docnumber'];
		...
		$consultationcode = 'TRG'.$consultationcode;
		}
         ...
         for ($p=1;$p<=20;$p++)
		  {	
			...
			$amount = $_REQUEST['amount'.$p];
				...
				{
					//echo '<br>'. 
					$query2 = "insert into master_consultationpharm(patientcode,patientname,patientvisitcode,medicinename,dose,frequencyauto_number,frequencycode,frequencynumber,days,quantity,instructions,rate,amount,recordstatus,recorddate,ipaddress,paymentstatus,medicinecode,refno,pharmacybill,medicineissue,store,location,billtype,accountname,locationname,locationcode) 
					values('$patientcode','$patientname','$visitcode','$medicinename','$dose','$frequencyautonumber','$frequencycode','$frequencynumber','$days','$quantity','$instructions','$rate','$amount','completed','$currentdate','$ipaddress','$status','$medicinecode','$pharefcode','$status','pending','$store','$location','$billingtype','$accountname','$locationname','$locationcode')";
					$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
					
					$query29 = "insert into master_consultationpharmissue(patientcode,patientname,patientvisitcode,medicinename,dose,frequencyauto_number,frequencycode,frequencynumber,days,quantity,instructions,rate,amount,recordstatus,recorddate,ipaddress,paymentstatus,medicinecode,refno,prescribed_quantity,store,location,billtype,accountname,locationname,locationcode) 
					values('$patientcode','$patientname','$visitcode','$medicinename','$dose','$frequencyautonumber','$frequencycode','$frequencynumber','$days','$quantity','$instructions','$rate','$amount','completed','$currentdate','$ipaddress','$status','$medicinecode','$pharefcode','$quantity','$store','$location','$billingtype','$accountname','$locationname','$locationcode')";
...
$maxanum = $labrefcode;
	...
	if($categorylabname != "")
	{
	$categorylabquery1=mysql_query("insert into consultation_lab(patientcode,patientname,patientvisitcode,labitemname,labitemrate,consultationdate,paymentstatus,labsamplecoll,resultentry,labrefund,refno,locationname,locationcode)values('walkin','$patientfullname','walkinvis','$categorylabname','$categorylabrate','$currentdate','paid','pending','pending','norefund','$labrefcode','$locationname','$locationcode')") or die(mysql_error());
	}	
	}
		...
		$labquery1=mysql_query("insert into consultation_lab(patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,consultationdate,paymentstatus,labsamplecoll,resultentry,labrefund,refno,billtype,accountname,username,locationname,locationcode,urgentstatus)values('$patientcode','$patientname','$visitcode','$labcode','$labname','$labrate','$currentdate','$status','pending','pending','norefund','$labrefcode','$billingtype','$accountname','$username','$locationname','$locationcode','$laburgent')") or die(mysql_error());
		}
				}
		}
		...
		if($num61 == 0)
		{
		$radiologyquery1=mysql_query("insert into consultation_radiology(patientcode,patientname,patientvisitcode,radiologyitemcode,radiologyitemname,radiologyitemrate,billtype,accountname,consultationdate,paymentstatus,refno,resultentry,username,locationname,locationcode)values('$patientcode','$patientname','$visitcode','$radiologycode','$pairvar','$pairvar1','$billingtype','$accountname','$currentdate','$status','$radrefcode','pending','$username','$locationname','$locationcode')") or die(mysql_error());
		}
		}
		}
		
		$query491 = "insert into master_triagebilling(patientcode,patientname,patientvisitcode,billtype,accountname,consultationdate,username,docnumber,locationname,locationcode)values('$patientcode','$patientname','$visitcode','$billingtype','$accountname','$currentdate','$username','$consultationcode','$locationname','$locationcode')";
		...
		$seramount = $servicesrate * $serviceqty;	
		
		if(($servicesname!="")&&($servicesrate!=''))
		{
		$servicesquery1=mysql_query("insert into consultation_services(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,consultationdate,paymentstatus,process,refno,billtype,accountname,username,locationname,locationcode,serviceqty,amount)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$currentdate','$status','pending','$serrefcode','$billingtype','$accountname','$username','$locationname','$locationcode','$serviceqty','$seramount')") or die(mysql_error());
			}
...
function grandtotalcalc()
{
	...
	var totgrand = parseFloat(total) + parseFloat(total1) + parseFloat(total2) + parseFloat(total3);
	totgrand = parseFloat(totgrand).toFixed(2);
	document.getElementById("grandtotal").value = totgrand;
}
...
<script>
window.onclick = function() { grandtotalcalc(); }
</script>
...
<form name="form1" id="frmsales" method="post" action="triagebilling.php" onKeyDown="return disableEnterKey(event)" onSubmit="return processcheck()">
  ...
  <tr>
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
			 ...
			 <tr>
				...
				?>
				 ...
				 <tr>
				  ...
				  <tr id="labid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				     <table width="673" border="0" cellspacing="1" cellpadding="1">
                     <tr>
                       <td width="414" class="bodytext3">Laboratory Test</td>
                       <td class="bodytext3">Rate</td>
                       <td width="40" class="bodytext3">&nbsp;</td>
					   <td width="148" class="bodytext3">&nbsp;</td>
					   <td width="148" class="bodytext3">&nbsp;</td>
                     </tr>
					  <tr>
					 <div id="insertrow1">					 </div></tr>
				      ...
				      <td width="414"><input name="lab[]" id="lab" type="text" size="69" autocomplete="off" ></td>
				      <td width="48"><input name="rate5[]" type="text" id="rate5" readonly size="8"></td>
					  <td><label>
                       <input type="button" name="Add1" id="Add1" value="Add" onClick="return insertitem2()" class="button" style="border: 1px solid #001E6A">
                       </label></td>
					   <td width="148" class="bodytext3"><input type="checkbox" name="laburgent[]" id="laburgent" value="1"></td>
					   <td class="bodytext3"> Urgent</td>
					   </tr>
				      </table>	  </td> 
				  </tr>
				  <tr>
		...
		<tr>
		 <td colspan="7" class="bodytext31" valign="center"  align="right"><strong>Grand Total </strong><input type="text" id="grandtotal" readonly size="7"></td>
		</tr>
