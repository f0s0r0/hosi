...
function ledgervalue($parentid,$ADate1,$ADate2,$tbinclude)
{
	...
	if($parentid != '')
	{
		...
		{  
			...
			$drresult = array();
			$querydr1bnk = "SELECT SUM(-1*debitamount) as journal FROM `master_journalentries` WHERE `ledgerid` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'
