...
if($dellocation!='')
{
	 $query="delete from master_services where locationcode='$dellocation'"; 
	$exec=mysql_query($query);
	header ("location:servicedataimport.php?upload=deleted");
}
...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$referencename = '';
			$referenceunit= $splitdelimiter[5];
			$referenceunit = strtoupper($referenceunit);
			//$referencerange= $splitdelimiter[8];
			$referencerange = '';
			$rateperunit = $splitdelimiter[3];
			$rate2 = $splitdelimiter[3];
			$slab = $splitdelimiter[4];
			$slab = strtoupper($slab);
			if($slab == 'YES') {
			$slab = '1'; }
			$baseunit = $splitdelimiter[5];
			$incrqty = $splitdelimiter[6];
			$incrrate = $splitdelimiter[7];
			$ipmarkup = $splitdelimiter[8];
			$pkg = $splitdelimiter[9];
			$pkg = strtolower($pkg);
			...
			{
				$query89 = "select * from master_services where itemcode = '$itemcode' and locationcode = '$location'";
				$exec89 = mysql_query($query89) or die ("Error in Query89".mysql_error());
				$row89 = mysql_num_rows($exec89);
				if($row89 == 0)
				{	
					$query2 = "insert into master_services (itemcode,itemname,categoryname,itemname_abbreviation,rateperunit,rate2,baseunit,locationcode,locationname, slab, incrementalquantity, incrementalrate, ipmarkup, pkg, ipaddress, updatetime) values ('$itemcode','$itemname','$categoryname','$itemname_abbreviation','$rateperunit','$rate2','$baseunit','$location','$locationname','$slab','$incrqty','$incrrate','$ipmarkup','$pkg','$ipaddress','$updatedatetime')";
	...
	header("location:servicedataimport.php?st=success");
