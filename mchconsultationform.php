...
$consultationdatetime = date('Y-m-d H:i:s');
...
$times=$_REQUEST['times'];
	...
	$ancfreerefer = isset($_POST["ancfreerefer"])? 1 : 0;
...
$updatedatetime = date('Y-m-d H:i:s');
	...
	if ($res2 == 0)
	{
        ...
        $query1 = "insert into master_consultationlist (visitcode,patientcode,patientfirstname,patientmiddlename,patientlastname,consultingdoctor,consultationtype,department,consultationdate,consultationtime,consultationfees,referredby,consultationremarks,visitcount,complaints,registrationdate,recordstatus,pulse,consultation,labitems,radiologyitems,serviceitems,refferal,consultationstatus,date,username) 
		values('$visitcode','$patientcode','$patientfirstname','$patientmiddlename','$patientlastname','$consultingdoctor','$consultationtype','$department','$consultationdatetime','$consultationdatetime','$consultationfees','$referredby','$consultationremarks','$visitcount','$complaints','$registrationdate','$recordstatus','$pulse','$consultation','$labitems','$radiologyitems','$serviceitems','$refferal','completed','$currentdate','$username')";
		...
		$query88 = "insert into master_consultation(consultation_id,patientcode,patientauto_number,patientname,patientvisitauto_number,patientvisitcode,recordstatus,recorddate,ipaddress,consultingdoctor,billtype,accountname,paymentstatus,sys,dia,pulse,temp,complaint,drugallergy,foodallergy,consultationtime) 
				values('$consultationid','$patientcode','$patientauto_number','$patientfullname','$patientvisit','$visitcode','completed','$currentdate','$ipaddress','$consultingdoctor','$billingtype','$accountname','pending','$bpsystolic','$bpdiastolic','$pulse','$celsius','$complaint','$drugallergy','$foodallergy','$timestamp')";
		...
		$disease111 = $_POST['dis'];
		$code111 = $_POST['code'];
		$items111 = array_combine($disease111,$code111);
		$pairs111 = array();
		...
		for ($p=1;$p<=20;$p++)
		{		
		   	 $number = $_REQUEST['number'.$p];
			 $pregnancyorder = $_REQUEST['order'.$p];
			 $year = $_REQUEST['year'.$p];
			 $notimesanc = $_REQUEST['anctimes'.$p];
			 $delivery = $_REQUEST['place'.$p];
			 $maturity = $_REQUEST['maturity'.$p];
			 $durationlabour = $_REQUEST['duration'.$p];
			 $typeofdelivery = $_REQUEST['type'.$p];
			 $birthweight = $_REQUEST['birthweight'.$p];
			 $sex = $_REQUEST['sex'.$p];
			 $outcome = $_REQUEST['outcome'.$p];
			 $puerperium = $_REQUEST['puerperium'.$p];
			
			if ($pregnancyorder != "")
			{			
		        $queryanc2 = "insert into ancprevpreg(patientname,patientcode,patientvisitcode,age,gender,subtype,number,pregnancyorder,year,notimesanc,delivery,maturity,durationlabour,typeofdelivery,
				birthweight,sex,outcome,puerperium,accountname,recorddate,recordtime,username,ipaddress) 
				values('$patientfullname','$patientcode','$visitcode','$age','$gender','$subtype','$number','$pregnancyorder','$year','$notimesanc','$delivery','$maturity','$durationlabour','$typeofdelivery',
				'$birthweight','$sex','$outcome','$puerperium','$accountname','$currentdate','$currenttime','$username','$ipaddress')";
				$execanc2 = mysql_query($queryanc2) or die ("Error in Queryanc2".mysql_error());				
			}
		
		}
		
		 $cvs = $_REQUEST['cvs'];
		 $vaiginalxam = $_REQUEST['vaginal'];
		 $breasts = $_REQUEST['breast'];
		 $dischargeulcer = $_REQUEST['disulcer'];
		 $abdomen = $_REQUEST['abdomen'];
		 if ($cvs != "")
			{			
		        $queryanc3 = "insert into ancphysixam(patientname,patientcode,patientvisitcode,age,gender,subtype,cvs,vaiginalxam,breasts,dischargeulcer,abdomen,accountname,recorddate,recordtime,username,ipaddress) 
				values('$patientfullname','$patientcode','$visitcode','$age','$gender','$subtype','$cvs','$vaiginalxam','$breasts','$dischargeulcer','$abdomen','$accountname','$currentdate','$currenttime','$username','$ipaddress')";
				$execanc3 = mysql_query($queryanc3) or die ("Error in Queryanc3".mysql_error());				
			}
		 
		for ($p=1;$p<=20;$p++)
		{		 
			 $number = $_REQUEST['no'.$p];  
			 $visit = $_REQUEST['visit'.$p];
			 $date = $_REQUEST['presdate'.$p];
			 $urine = $_REQUEST['urine'.$p];
			 $weight = $_REQUEST['presweight'.$p];
			 $bp = $_REQUEST['bp'.$p];
			 $hp = $_REQUEST['hp'.$p];
			 $pallor = $_REQUEST['pallor'.$p];
			 $maturity = $_REQUEST['presmaturity'.$p];
			 $fundalheight = $_REQUEST['fundalweight'.$p];
			 $presentation = $_REQUEST['present'.$p];
			 $lie = $_REQUEST['lie'.$p];
			 $foetalheart = $_REQUEST['foetalheart'.$p];
			 $nextvist = $_REQUEST['nextvisit'.$p];
			
			if ($visit != "")
			{			
		        $queryanc4 = "insert into ancprespreg(patientname,patientcode,patientvisitcode,age,gender,subtype,number,visit,date,urine,weight,bp,hp,pallor,
				maturity,fundalheight,presentation,lie,foetalheart,nextvist,accountname,recorddate,recordtime,username,ipaddress) 
				values('$patientfullname','$patientcode','$visitcode','$age','$gender','$subtype','$number','$visit','$date','$urine','$weight','$bp','$hp','$pallor',
				'$maturity','$fundalheight','$presentation','$lie','$foetalheart','$nextvist','$accountname','$currentdate','$currenttime','$username','$ipaddress')";
				$execanc4 = mysql_query($queryanc4) or die ("Error in Queryanc4".mysql_error());				
			}
		...
		for ($p=1;$p<=20;$p++)
		{		   
			 $services = $_REQUEST['preventservice'.$p];
			 $date = $_REQUEST['preventdate'.$p];
			 $visit = $_REQUEST['preventvisit'.$p];

			
			if ($services != "")
			{			
		        $queryanc5 = "insert into ancprevser(patientname,patientcode,patientvisitcode,age,gender,subtype,services,visit,date,accountname,recorddate,recordtime,username,ipaddress) 
				values('$patientfullname','$patientcode','$visitcode','$age','$gender','$subtype','$services','$visit','$date','$accountname','$currentdate','$currenttime','$username','$ipaddress')";
...
$maxanum = $labrefcode;
	...
	$categorylabquery1=mysql_query("insert into consultation_lab(consultationid,patientcode,patientname,patientvisitcode,labitemname,labitemrate,billtype,accountname,consultationdate,paymentstatus,refno,labsamplecoll,resultentry,labrefund)values('$consultationid','$patientcode','$patientfullname','$visitcode','$categorylabname','$categorylabrate','$billingtype','$accountname','$currentdate','$status','$labrefcode','pending','pending','norefund')") or die(mysql_error());
		...
		if($labfree=='Yes'){$mchstatus = 'completed'; }else{$mchstatus = $status;}
		if(($labname!="")&&($labrate!=''))
		{
		...
		$labquery1=mysql_query("insert into consultation_lab(consultationid,patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,billtype,accountname,consultationdate,paymentstatus,refno,labsamplecoll,resultentry,labrefund,urgentstatus,consultationtime,freestatus,username)values('$consultationid','$patientcode','$patientfullname','$visitcode','$labcode','$labname','$labrate','$billingtype','$accountname','$currentdate','$mchstatus','$labrefcode','pending','pending','norefund','$urgentstatus','$timestamp','$labfree','$username')") or die(mysql_error());
			}
			}
		}
		...
		$serviceamount=$_POST['serviceamount'][$key];
		...
		if($departmentreferal != '')
		{
		$newquery3=mysql_query("update master_visitentry set referalconsultation='' where visitcode='$visitcode'");
		}
		else
		{
		$newquery3=mysql_query("update master_visitentry set referalconsultation='completed' where visitcode='$visitcode'");
		}
		}
		...
		header ("location:mchconsultationlist.php?patientcode=$patientcode&&st=success");
		exit;
		}
	}
	else
...
<script type="text/javascript" src="js/insertreferratemch.js"></script>
<script type="text/javascript" src="js/insertnewitemmchlab.js"></script>
<script type="text/javascript" src="js/insertnewitem25.js"></script>
<script type="text/javascript" src="js/insertnewitem26.js"></script>
<script type="text/javascript" src="js/insertnewitem27.js"></script>
...
function btnDeleteClick(delID,pharmamount)
{
	...
	var newgrandtotal4=parseFloat(newtotal4)+parseFloat(totalamount11)+parseFloat(totalamount21)+parseFloat(totalamount31)+parseFloat(totalamount41);
...
function btnDeleteClick1(delID1,vrate1)
{
	...
	else
	{
	 totalamount41=document.getElementById('total5').value;
		 ...
		 newgrandtotal3=parseFloat(totalamount11)+parseFloat(newtotal3)+parseFloat(totalamount21)+parseFloat(totalamount31)+parseFloat(totalamount41);
...
function btnDeleteClick5(delID5,radrate)
{
    ...
    var newgrandtotal2=parseFloat(totalamount11)+parseFloat(totalamount21)+parseFloat(newtotal2)+parseFloat(totalamount31)+parseFloat(totalamount41);
...
var currenttotal1=document.getElementById('total3').value;
	...
	var newgrandtotal1=parseFloat(totalamount11)+parseFloat(totalamount21)+parseFloat(totalamount31)+parseFloat(newtotal1)+parseFloat(totalamount41);
...
function btnDeleteClick25(delID25)
{
	//alert ("Inside btnDeleteClick.");
	//var newtotal;
	//alert(delID4);
	var varDeleteID25= delID25;
	//alert(varDeleteID4);
	
	//alert(rateref);
	//alert (varDeleteID3);
	var fRet25; 
	fRet25 = confirm('Are You Sure Want To Delete This Entry?'); 
	//alert(fRet); 
	if (fRet25 == false)
	{
		//alert ("Item Entry Not Deleted.");
	...
	var child25 = document.getElementById('idTRc'+varDeleteID25);  
	//alert (child3);//tr name
    var parent25 = document.getElementById('ancinsertrow'); // tbody name.
	document.getElementById ('ancinsertrow').removeChild(child25);
...
function btnDeleteClick26(delID26)
{
	//alert ("Inside btnDeleteClick.");
	//var newtotal;
	//alert(delID4);
	var varDeleteID26= delID26;
	//alert(varDeleteID4);
	
	//alert(rateref);
	//alert (varDeleteID3);
	var fRet26; 
	fRet26 = confirm('Are You Sure Want To Delete This Entry?'); 
	//alert(fRet); 
	if (fRet26 == false)
	{
		//alert ("Item Entry Not Deleted.");
	...
	var child26 = document.getElementById('idTRb'+varDeleteID26);  
	//alert (child26);//tr name
    var parent26 = document.getElementById('preinsertrow1'); // tbody name.
	document.getElementById ('preinsertrow1').removeChild(child26);
	// tbody name.
	
	
	
}
function btnDeleteClick27(delID27)
{
	//alert ("Inside btnDeleteClick.");
	//var newtotal;
	//alert(delID4);
	var varDeleteID27= delID27;
	//alert(varDeleteID27);
	
	//alert(rateref);
	//alert (varDeleteID3);
	var fRet27; 
	fRet27 = confirm('Are You Sure Want To Delete This Entry?'); 
	//alert(fRet); 
	if (fRet27 == false)
	{
		//alert ("Item Entry Not Deleted.");
	...
	var child27 = document.getElementById('idTRa'+varDeleteID27);  
	//alert (child27);//tr name
    var parent27 = document.getElementById('preventinsertrow'); // tbody name.
	document.getElementById ('preventinsertrow').removeChild(child27);
...
function funcancShowView()
{
  if (document.getElementById("ancid") != null) 
     {
	 document.getElementById("ancid").style.display = 'none';
	}
	if (document.getElementById("ancid") != null) 
	  {
	  document.getElementById("ancid").style.display = '';
	 }
	 if (document.getElementById("ancid1") != null) 
     {
	 document.getElementById("ancid1").style.display = 'none';
	}
	if (document.getElementById("ancid1") != null) 
	  {
	  document.getElementById("ancid1").style.display = '';
	 }
	 if (document.getElementById("ancid2") != null) 
     {
	 document.getElementById("ancid2").style.display = 'none';
	}
	if (document.getElementById("ancid2") != null) 
	  {
	  document.getElementById("ancid2").style.display = '';
	 }
	 if (document.getElementById("ancid3") != null) 
     {
	 document.getElementById("ancid3").style.display = 'none';
	}
	if (document.getElementById("ancid3") != null) 
	  {
	  document.getElementById("ancid3").style.display = '';
	 }
	 
}
function funcancHideView()
{		
 if (document.getElementById("ancid") != null) 
	{
	document.getElementById("ancid").style.display = 'none';
...
function Tempcheck()
{
if(document.form1.celsius.value > 37.4 && document.form1.celsius.value !="")
	{
...
<table width="103%" border="0" cellspacing="0" cellpadding="2">
  ...
  <tr>
    ...
    <td width="97%" valign="top">
     <form name="form1" id="form1" method="post" action="mchconsultationform.php" onSubmit="return process1();">
            ...
            <tbody>
              <tr bgcolor="#011E6A">
                <td colspan="6" bgcolor="#CCCCCC" class="style2">Consultation</td>
				 <td colspan="4" align="center" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong><a target="_blank" href="emrresultsviewlist.php?patientcode=<?php echo $patientcode; ?>">EMR</a></strong></td>
...
-->
				...
				<td colspan="6" align="left" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo ''; } else { echo '#AAFF00'; } ?>" class="bodytext3"><?php echo $errmsg;?>&nbsp;</td>
				 ...
				 <tr bgcolor="">
				   ...
				   <td align="left" valign="middle"  bgcolor="" class="bodytext3"><label><?php echo $height;?>
                     <input type="hidden" name="height" id="height" value="<?php echo $height;?>" size="10" onKeyUp="return FunctionBMI()" onKeyPress="return isNumber(event)">
				   ...
				   <td align="left" valign="middle"  bgcolor=""><span class="bodytext32"><?php echo $weight?>
                       <input type="hidden" name="weight" id="weight" value="<?php echo $weight?>" size="10" onKeyUp="return FunctionBMI()" onKeyPress="return isNumber(event)">
                   </span></td>
				   ...
				   <td align="left" valign="middle"  bgcolor=""><span class="bodytext32"><?php echo $bmi?>
                       <input name="bmi" type="hidden" id="bmi"  value="<?php echo $bmi?>" readonly  size="10">
                   </span></td>
			      </tr>
				  <tr bgcolor="">
				    <td align="left" valign="middle"  bgcolor="" class="bodytext3"><span class="bodytext32"><strong>Sys</strong></span> </td>
					  <td align="left" valign="middle"  bgcolor="" class="bodytext3"><span class="bodytext32"><?php echo $bpsystolic;?>
                          <input name="bpsystolic" type="hidden" id="bpsystolic" value="<?php echo $bpsystolic;?>" onBlur="return MinimumBP()" onKeyPress="return isNumber(event)" size="10">
					  </span> </td>
					  ...
					  <td align="left" valign="middle"  bgcolor="" class="bodytext3"><span class="bodytext32"><?php echo $bpdiastolic;?>
                          <input name="bpdiastolic" type="hidden" id="bpdiastolic" value="<?php echo $bpdiastolic;?>"onBlur="return MaximumBP()" size="10" onKeyPress="return isNumber(event)">
					  </span></td>
					  ...
					  <td align="left" valign="middle"  bgcolor="" class="bodytext3"><span class="bodytext32"><?php echo $pulse;?>
                          <input name="pulse" type="hidden" id="pulse" value="<?php echo $pulse;?>" size="10">
                      </span></td>
			      </tr>
				 <tr bgcolor="">
				   ...
				   <td align="left" valign="middle"  bgcolor="" class="bodytext3"><span class="bodytext32"><?php echo $respiration;?>
                       <input name="respiration" type="hidden" value="<?php echo $respiration;?>" id="respiration" size="10">
				   </span></td>
				   ...
				   <td align="left" valign="middle"  bgcolor=""><span class="bodytext32"><?php echo number_format($celsius,2,'.','');?>
				     <input name="celsius" type="hidden" onKeyUp="return FunctionTemperature()" onBlur="return Tempcheck()" id="celsius" value="<?php echo $celsius;?>" size="10">
				   </span></td>
				   ...
				   <td align="left" valign="middle"  bgcolor="">
                    ...
                    <input type="hidden" name="fahrenheit" id="fahrenheit" value="<?php echo $fahrenheit;?>" size="10">
		            </span></label></td>
		          </tr>
				 <tr bgcolor="">
				 ...
				 <td align="left" valign="middle"  bgcolor=""><label class="bodytext32">
				  ...
				  <tr>
				 <td align="left" valign="middle"  bgcolor=""><span class="bodytext32"><strong>Subsequent Visit</strong></span></td>
				 <td align="left" valign="middle"  bgcolor=""><label class="bodytext32">
				     <?php if($gestationage !=''){ ?>
				   GestationAge (<label style="color:red;"><?php echo $gestationage; ?></label>)&nbsp;
				   <?php } ?>
				    <?php if($noofvisit !=''){ ?>
				   NoofVisit (<label style="color:red;"><?php echo $noofvisit; ?></label>)&nbsp;
				   <?php } ?>
				    <?php if($urinedipstict !=''){ ?>
				   Urinedipstict (<label style="color:red;"><?php echo $urinedipstict; ?></label>)&nbsp;
				   <?php } ?>             </td>
				  <td align="left" valign="middle"  bgcolor="" class="bodytext3"><span class="bodytext32">
				  <strong>ANC Profile</strong></span></td>
				     <td colspan="3" align="left" valign="middle"  bgcolor="" class="bodytext3"><span class="bodytext32">
					  <?php if($bloodgroup !=''){ ?>
				   BloodGroup (<label style="color:red;"><?php echo $bloodgroup; ?></label>)&nbsp;
				   <?php } ?>
				    <?php if($hblevel !=''){ ?>
				   HB Level (<label style="color:red;"><?php echo $hblevel; ?></label>)&nbsp;
				   <?php } ?>
				    <?php if($vdrl !=''){ ?>
				   VDRL (<label style="color:red;"><?php echo $vdrl; ?></label>)&nbsp;
				   <?php } ?>
				   <?php if($pmtct !=''){ ?>
				   PMTCT (<label style="color:red;"><?php echo $pmtct; ?></label>)&nbsp;
				   <?php } ?>
				   <?php if($urinalysis !=''){ ?>
				   Urinalysis (<label style="color:red;"><?php echo $urinalysis; ?></label>)&nbsp;
				   <?php } ?>
				   </span></td>
			      </tr>
				   <tr>
				 ...
				 <tr>
				  ...
				  <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><strong>ANC Consultation</strong> <span class="bodytext32"> <img src="images/plus1.gif" width="13" height="13"   onDblClick="return funcancHideView()" onClick="return funcancShowView()"> </span></td>
			      </tr>
				  <tr id="ancid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				   <table id="presid" width="831" border="0" cellspacing="1" cellpadding="1">
					 ...
					 $colorloopcount='';
					 $showcolor='';
					 $queryanc5 = "select * from ancprevpreg where patientcode = '$patientcode'";
					 $execanc5 = mysql_query($queryanc5) or die ("Error in Queryanc5".mysql_error());
					...
					else
					{
					 ...
					 <td width="54" class="bodytext3"><?php echo $resanc5['number']; ?></td>
                       <td width="54" class="bodytext3"><?php echo $resanc5['pregnancyorder'];?></td>
                       <td width="48" class="bodytext3"><?php echo $resanc5['year'];?></td>
                       <td width="48" class="bodytext3"><?php echo $resanc5['notimesanc'];?></td>
                       <td width="90" class="bodytext3"><?php echo $resanc5['delivery'];?></td>
                       <td width="73" class="bodytext3"><?php echo $resanc5['maturity'];?></td>
					    <td width="65" class="bodytext3"><?php echo $resanc5['durationlabour'];?></td>
                       <td width="120" class="bodytext3"><?php echo $resanc5['typeofdelivery'];?></td>
                       <td width="78" class="bodytext3"><?php echo $resanc5['birthweight'];?></td>
                       <td width="50" class="bodytext3"><?php echo $resanc5['sex'];?></td>
                       <td width="51" class="bodytext3"><?php echo $resanc5['outcome'];?></td>
					    <td width="54" class="bodytext3"><?php echo $resanc5['puerperium'];?></td>
                     </tr>
					 <?php }
					 ?>
					 <td width="54" class="bodytext3">Number</td>
                       <td width="54" class="bodytext3">Pregnancy Order</td>
                       <td width="48" class="bodytext3">Year</td>
                       <td width="48" class="bodytext3">Number of times ANC attended</td>
                       <td width="90" class="bodytext3">Place of Delivery</td>
                       <td width="73" class="bodytext3">Maturity</td>
					    <td width="65" class="bodytext3">Duration of Labour</td>
                       <td width="120" class="bodytext3">Type of Delivery</td>
                       <td width="78" class="bodytext3">Birth Weight Kg</td>
                       <td width="50" class="bodytext3">Sex</td>
                       <td width="51" class="bodytext3">Outcome</td>
					    <td width="54" class="bodytext3">Puerperium</td>
                     </tr>
					 <tr>
					 <div id="ancinsertrow">					 </div></tr>
                     <tr>
					  <input type="hidden" name="serialnumber6" id="serialnumber6" value="1">
					  	<td><input name="number" type="text" id="number" size="8" autocomplete="off">					   </td>	
                       <td><input name="order" type="text" id="order" size="8" autocomplete="off">					   </td>
                       <td><input name="year" type="text" id="year" size="8"></td>
                       <td><input name="anctimes" type="text" id="anctimes" size="8">
		   				</td>	
                       <td><input name="place" type="text" id="place" size="8"></td>
                       <td><input name="maturity" type="text" id="maturity" size="8" ></td>
					   <td><input name="duration" type="text" id="duration" size="8" ></td>
                       <td><input name="type" type="text" id="type" size="20"></td>
                       <td><input name="birthweight" type="text" id="birthweight"  size="8"></td>
                       <td>
                         <input name="sex" type="text" id="sex"  size="8"></td>
						  <td>
						 
                         <input name="outcome" type="text" id="outcome"  size="8"></td>
						 <td>
                         <input name="puerperium" type="text" id="puerperium"  size="8"></td>
                       <td width="63"><label>
                       <input type="button" name="Add" id="Add" value="Add" onClick="return insertitem25()" class="button" >
                       </label></td>
				      </tr>
				   ...
				   <tr><td colspan="10" class="bodytext3"><strong>Physical Examination ( First Visit)</strong></td></tr>
				    <?php
					 $queryanc6 = "select * from ancphysixam where patientcode = '$patientcode'";
					...
					else
					{
					 ...
					 ?>	
				   <tr <?php echo $colorcode ; ?>>
					 <td colspan="2" class="bodytext3"><?php echo $resanc6['cvs']; ?></td>                       
                       <td colspan="2" class="bodytext3"><?php echo $resanc6['vaiginalxam']; ?></td>
                       
                       <td colspan="2"class="bodytext3"><?php echo $resanc6['breasts']; ?></td>
                       
					   <td colspan="2" class="bodytext3"><?php echo $resanc6['dischargeulcer']; ?></td>
                       
                       <td colspan="2" class="bodytext3"><?php echo $resanc6['abdomen']; ?></td>
				      </tr>
					   <?php }
					 ?>
                     <tr>
					 <td width="54" class="bodytext3">CVS</td>
                       <td width="54" class="bodytext3"><textarea id="cvs" name="cvs" ></textarea></td>
                       <td width="48" class="bodytext3">Vaginal Examination</td>
                       <td width="48" class="bodytext3"><textarea id="vaginal" name="vaginal" ></textarea></td>
                       <td width="90" class="bodytext3">Breasts</td>
                       <td width="73" class="bodytext3"><textarea id="breast" name="breast" ></textarea></td>
					   <td width="65" class="bodytext3">Discharge/Genital Ulcer</td>
                       <td width="120" class="bodytext3"><textarea id="disulcer" name="disulcer" ></textarea></td>
                       <td width="78" class="bodytext3">Abdomen</td>
                       <td width="50" class="bodytext3"><span class="bodytext32">
				   ...
				   <table id="presid" width="900" border="0" cellspacing="1" cellpadding="1">
				    <tr><td colspan="14" class="bodytext3"><strong>Present Pregnancy</strong></td></tr>
					 <?php
					 $sno='';
					 $queryanc7 = "select * from ancprespreg where patientcode = '$patientcode'";
					...
					else
					{
					 ...
					 ?>	
                     <tr <?php echo $colorcode ; ?>>
					 <td width="33" class="bodytext3"><?php echo $resanc7['number']; ?></td>
                       <td width="50" class="bodytext3"><?php echo $resanc7['visit']; ?></td>
                       <td width="103" class="bodytext3"><?php echo $resanc7['date']; ?></td>
                       <td width="63" class="bodytext3"><?php echo $resanc7['urine']; ?></td>
                       <td width="43" class="bodytext3"><?php echo $resanc7['weight']; ?></td>
                       <td width="24" class="bodytext3"><?php echo $resanc7['bp']; ?></td>
					    <td width="37" class="bodytext3"><?php echo $resanc7['hp']; ?></td>
                       <td width="75" class="bodytext3"><?php echo $resanc7['pallor']; ?></td>
                       <td width="61" class="bodytext3"><?php echo $resanc7['maturity']; ?></td>
                       <td width="48" class="bodytext3"><?php echo $resanc7['fundalheight']; ?></td>
                       <td width="74" class="bodytext3"><?php echo $resanc7['presentation']; ?></td>
					    <td width="50" class="bodytext3"><?php echo $resanc7['lie']; ?></td>
						 <td width="50" class="bodytext3"><?php echo $resanc7['foetalheart']; ?></td>
						  <td width="50" class="bodytext3"><?php echo $resanc7['nextvist']; ?></td>
                     </tr>
					 <?php } ?>
					  <tr>
					 <td width="33" class="bodytext3">No</td>
                       <td width="50" class="bodytext3">Visit</td>
                       <td width="103" class="bodytext3">Date</td>
                       <td width="63" class="bodytext3">Urine</td>
                       <td width="43" class="bodytext3">Weight</td>
                       <td width="24" class="bodytext3">BP</td>
					    <td width="37" class="bodytext3">HP</td>
                       <td width="75" class="bodytext3">Pallor</td>
                       <td width="61" class="bodytext3">Maturity</td>
                       <td width="48" class="bodytext3">Fundal Height</td>
                       <td width="74" class="bodytext3">Presentation</td>
					    <td width="50" class="bodytext3">Lie</td>
						 <td width="50" class="bodytext3">Foetal Heart</td>
						  <td width="50" class="bodytext3">Next Visit</td>
                     </tr>
					 <tr>
					 <div id="preinsertrow1">					 </div></tr>
                     <tr>
					  <input type="hidden" name="presserialnumber1" id="presserialnumber1" value="1">
					  	<td><input name="no" type="text" id="no" size="3" autocomplete="off">					   </td>	
                       <td><input name="visit" type="text" id="visit" size="8" autocomplete="off">					   </td>
                       <td style="position:relative;"><input name="presdate" type="text" id="presdate" size="10">
					   <img src="images2/cal.gif" id="presimgdate" onClick="javascript:NewCssCal('presdate')" style="cursor:pointer; position: absolute;top: 5px;left: 74px;"/></td>
                       <td><input name="urine" type="text" id="urine" size="8">
		   				</td>	
                       <td><input name="presweight" type="text" id="presweight" size="3"></td>
                       <td><input name="bp" type="text" id="bp" size="3" ></td>
					   <td><input name="hp" type="text" id="hp" size="3" ></td>
                       <td><input name="pallor" type="text" id="pallor" size="8"></td>
                       <td><input name="presmaturity" type="text" id="presmaturity"  size="8"></td>
                       <td>
                         <input name="fundalweight" type="text" id="fundalweight"  size="8"></td>
						  <td>
						 
                         <input name="present" type="text" id="present"  size="8"></td>
						 <td>
                         <input name="lie" type="text" id="lie"  size="8"></td>
						 <td>
                         <input name="foetalheart" type="text" id="foetalheart"  size="8"></td>
						 <td>
                         <input name="nextvisit" type="text" id="nextvisit"  size="8"></td>
                       <td width="54"><label>
                       <input type="button" name="Add" id="Add" value="Add" onClick="return insertitem26()" class="button" >
                       </label></td>
				      </tr>
				   ...
				   <table id="presid" width="831" border="0" cellspacing="1" cellpadding="1">
				    <tr><td colspan="5" class="bodytext3"><strong>Preventive Services</strong></td></tr>
					 <?php
					 $sno='';
					 $queryanc8 = "select * from ancprevser where patientcode = '$patientcode'";
					...
					else
					{
					 ...
					 <td width="132" class="bodytext3">Services</td>
                       ...
                       <td width="502" class="bodytext3"></td>
                      
                     </tr>
					 <tr>
					 <div id="preventinsertrow">					 </div></tr>
                     <tr>
					  <input type="hidden" name="preventserialnumber6" id="preventserialnumber6" value="1">
					  	<td>
						<select name="preventservice" id="preventservice">
						<option value="">Select</option> 
						<option value="Tetanus toxoid 1">Tetanus toxoid 1</option> 
						<option value="Tetanus toxoid 2">Tetanus toxoid 2</option> 
						<option value="Tetanus toxoid 3">Tetanus toxoid 3</option> 
						<option value="Tetanus toxoid 4">Tetanus toxoid 4</option> 
						<option value="Tetanus toxoid 5">Tetanus toxoid 5</option> 
						</select>
						</td>	
                       <td><input name="preventdate" type="text" id="preventdate" size="8" autocomplete="off">	
					   <img src="images2/cal.gif" onClick="javascript:NewCssCal('preventdate')" style="cursor:pointer"/>				   </td>
                       <td><input name="preventvisit" type="text" id="preventvisit" size="8"></td>                       
                       <td width="502"><label>
                       <input type="button" name="Add" id="Add" value="Add" onClick="return insertitem27()" class="button" >
                       </label></td>
				      </tr>
				   ...
				   <tr>
				   <td colspan="8" align="right" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><span class="style2">&nbsp;</span><span class="bodytext32">
				...
				?>
				 ...
				 <tr id="labid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
					 ...
					 <div id="insertrow1">					 </div></tr>
					  ...
					  <td><label>
                       <input type="button" name="Add1" id="Add1" value="Add" onClick="return insertitem20()" class="button" >
                       </label></td>
				 ...
				 <tr id="reffid">
					...
					<tr id="ancfree">
					<td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong>Free Gyne Consultation</strong> <input type="checkbox" name="ancfreerefer" id="ancfreerefer" onClick="return insertitemr();"></td>
					</tr>
                     <!--<tr>
		         ...
		         <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><span class="style2">Special Clinic <img src="images/plus1.gif" width="13" height="13"  onDblClick="return  funcExRefferalHideView()" onClick="return funcExRefferalShowView()"> </span></span></td>
		        </tr>
				<tr id="exreffid">
				 <td colspan="8" align="center" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><a target="_blank" href="appointmentscheduleentry.php?patientcode=<?php echo $patientcode; ?>">Special Clinic Appointment</a></td>
				</tr>
				 <tr id="exreffid1">
				    <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				    <table id="presid" width="417" border="0" cellspacing="1" cellpadding="1">
					 ...
					 <div id="insertrow4">					 </div></tr>
				      ...
				      <td><input name="rate4[]" type="text" id="rate4" size="8"></td>
					   
                     <td>  <input type="button" name="Add4" id="Add4" value="Add" onClick="return insertitem5()" class="button">
                      </td>
					   </tr>
				      </table></td>
						 <tr>
				   <td colspan="8" align="right" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><span class="style2">Total</span>
				  <input type="text" id="total5" readonly size="7"></td></tr>
                ...
                <td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><input type="checkbox" name="ipadmit" id="ipadmit" onClick="return notescheck();"></td>
