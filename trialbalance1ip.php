    ...
    <td colspan="2" valign="top"><table width="99%" border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		<table width="1202" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
          <tbody>
            <tr bgcolor="#011E6A">
              <td colspan="4" bgcolor="#CCCCCC" class="bodytext3"><strong> Trial Balance IP</strong></td>
              </tr>
             <tr>
	...
	if($parentid != '' && $tbinclude != '')
	{
		...
		{  
			...
			{					 
				...
				$j = 0;
				$crresult = array();
				$querycr1dp = "SELECT SUM(amount) as deposit FROM `deposit_refund` WHERE visitcode IN (select visitcode FROM `master_transactionip` WHERE transactiondate BETWEEN '$ADate1' AND '$ADate2' group by billnumber)
							 UNION ALL SELECT SUM(amount) as deposit FROM `deposit_refund` WHERE visitcode IN (select visitcode FROM `billing_ipcreditapprovedtransaction` WHERE billdate BETWEEN '$ADate1' AND '$ADate2' group by billno)
			...
			else
			{
				...
				$querydr1bnk = "SELECT SUM(`cashamount`) as bankamount FROM `master_transactionadvancedeposit` WHERE `cashcode` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
									...
									UNION ALL SELECT SUM(`transactionamount`) as bankamount FROM `master_transactionip` WHERE `accountnameid` = '$id' AND billtype = 'PAY LATER' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'";
