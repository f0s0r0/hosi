...
$ratecolumn = $locationcode.'_rateperunit';
	...
	$query571 = "select itemcode from transaction_stock where locationcode='".$locationcode."' AND itemname like '%$searchmedicinename1%' and batch_stockstatus='1' group by itemcode";
	$res571=mysql_query($query571);
	while($exec571 = mysql_fetch_array($res571))
	{
	$stockitemcode = $exec571['itemcode'];
	
	//include ('autocompletestockcount1include1.php');
	$query57 = "select sum(batch_quantity) as currentstock from transaction_stock where locationcode='".$locationcode."' AND itemcode = '$stockitemcode' and batch_stockstatus='1'";
	
	$res57=mysql_query($query57);
	$exec57 = mysql_fetch_array($res57);
	$currentstock = $exec57['currentstock'];
	
	$query200 = "select itemcode,itemname,disease,genericname,pkg,`$ratecolumn` from master_medicine where itemcode = '$stockitemcode' and status <> 'deleted' order by itemname limit 0, 20";
