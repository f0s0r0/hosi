...
$res7locationanum = $res23['location'];
 ...
 $query7=mysql_query("select * from master_planname where auto_number='$planname'");
...
$rowcount111 = mysql_num_rows($exec111);
...
if ($frmflag1 == 'frmflag1')
{
   ...
   $exec222=mysql_query($query222) or die(mysql_error());
	...
	{
	if(($planpercentage > 0.00 && $forall != '') || ($grandtotal1 > $availablelimit))
		{
			$status='pending';
			}
			else
...
$times=$_REQUEST['times'];
	...
	$requestsamount = $_REQUEST['total4'];
	...
	//$querymur="UPDATE `master_consultation` SET murphing='".$murphing."' WHERE patientcode='".$patientcode."' and patientvisitcode='".$visitcode."' " ;
//	$execmur=mysql_query($querymur);
...
$updatedatetime = date('Y-m-d H:i:s');
	...
	if ($res2 == 0)
	{
		...
		if($num70 == 0)
		{
		$icdquery2 = "insert into consultation_icd1(consultationid,patientcode,patientname,patientvisitcode,disease,icdcode,accountname,consultationdate,consultationtime,locationname,locationcode,tablefrom)
		values('$consultationid','$patientcode','$patientfullname','$visitcode','$pairvar113','$pairvar114','$accountname','$currentdate','$timestamp','".$locationnameget."','".$locationcodeget."','$tablefrom')";
		...
		$query88 = "insert into master_consultation(consultation_id,patientcode,patientauto_number,patientname,patientvisitauto_number,patientvisitcode,recordstatus,recorddate,ipaddress,consultingdoctor,billtype,accountname,paymentstatus,sys,dia,pulse,temp,complaint,murphing,drugallergy,foodallergy,consultationtime,requestsamount,locationname,locationcode) 
				values('$consultationid','$patientcode','$patientauto_number','$patientfullname','$patientvisit','$visitcode','completed','$currentdate','$ipaddress','$consultingdoctor','$billingtype','$accountname','pending','$bpsystolic','$bpdiastolic','$pulse','$celsius','$complaint','$murphing','$drugallergy','$foodallergy','$timestamp','$requestsamount','".$locationnameget."','".$locationcodeget."')";
				$exec88 = mysql_query($query88) or die ("Error in Query88".mysql_error());
...
$maxanum = $labrefcode;
	...
	if($categorylabname != "")
	{
	$categorylabquery1=mysql_query("insert into consultation_lab(consultationid,patientcode,patientname,patientvisitcode,labitemname,labitemrate,billtype,accountname,consultationdate,paymentstatus,refno,labsamplecoll,resultentry,labrefund,locationname,locationcode)values('$consultationid','$patientcode','$patientfullname','$visitcode','$categorylabname','$categorylabrate','$billingtype','$accountname','$currentdate','$status','$labrefcode','pending','pending','norefund','".$locationnameget."','".$locationcodeget."')") or die(mysql_error());
	}	
	}
...
if(($billingtype =='PAY LATER') && ($planpercentage==0))
	...
	//{ 
		...
		$servicesquery1=mysql_query("insert into consultation_services(consultationid,patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,billtype,accountname,consultationdate,paymentstatus,refno,process,consultationtime,username,locationname,locationcode,serviceqty,amount)values('$consultationid','$patientcode','$patientfullname','$visitcode','$servicescode','$servicesname','$servicesrate','$billingtype','$accountname','$currentdate','$status','$serrefcode','pending','$timestamp','$username','$locationnameget','$locationcodeget','$serviceqty','".$serviceamount."')") or die(mysql_error());
		}
	//}
		...
		//this is for checking pay now or pay later
		if(($billingtype =='PAY LATER') && ($planpercentage==0))
	{
	if($grandtotal1 > $availablelimit)
		{
	...
	//********************
		...
		header ("location:reviewlist.php?patientcode=$patientcode&&st=success");
		exit;
		}
	}
	else
...
$viscode=$_REQUEST['visitcode'];
 ...
 $viscode=""; }
    ...
    $query2 = "select * from master_triage where patientcode = '$patientcode' and visitcode='$viscode' order by auto_number desc";//order by auto_number desc limit 0, 1";
		...
		if($locationcode == ''){ $locationcode = 'LTC-1'; }
		 //location end
...
$visitlimit = $res59['visitlimit'];
$consultationfees = $res59['consultationfees'];
$gender = $res59['gender'];
$feetype= $res59['paymenttype'];

$query01=mysql_query("select paymenttype from master_paymenttype where auto_number='$feetype'");
$res01=mysql_fetch_array($query01);
$feetypename= $res01['paymenttype'];
...
$query591 = "select * from billing_consultation where patientvisitcode='$viscode'";
$exec591 = mysql_query($query591) or die(mysql_error());
$num591 = mysql_num_rows($exec591);

if($num591 > 0)
{
$res591 = mysql_fetch_array($exec591);
$paidfees = $res591['consultation'];
}
else
{
$paidfees = 0;
...
$query592 = "select sum(requestsamount) as requestsamount from master_consultation where patientvisitcode='$viscode'";
$exec592 = mysql_query($query592) or die(mysql_error());
$res592 = mysql_fetch_array($exec592);
$requestsamount = $res592['requestsamount'];

$consultationfees = $consultationfees - $paidfees;
if($visitlimit != 0)
{
$visitbalance = $visitlimit - $consultationfees - $requestsamount;
}
else
{
$visitbalance = 0;
...
function sertotal()
{
	...
	if(varserSlab=='')
	{
		...
		if(parseInt(varquantityser)>0)
		{
		document.getElementById("serviceamount").value=(parseInt(varserRates)*parseInt(varquantityser)).toFixed(2);
		}
		}
	if(parseInt(varserSlab)==1)
	{
		...
		if (parseInt(varquantityser) > parseInt(varserBaseunit))
		{
			...
			document.getElementById("serviceamount").value=parseInt(varserRates)+parseInt(resultfinal);
		}
	}
...
function process1()
{
	...
	var visitbalance = document.getElementById("visitbalance").value;
	var grandtotal =  document.getElementById("total4").value;
	var visitlimit = document.getElementById("visitlimit").value;

	if(visitlimit != 0)
	{
		if(parseInt(grandtotal) > parseInt(visitbalance))
		{
		alert("Patient Visit Limit is Exceeded. Pl Modify Requests");
		return false;
		}
...
var var11=document.getElementById("total4").value;
...
{
  	var oTextbox1 = new AutoSuggestControl1(document.getElementById("lab"), new StateSuggestions1()); 
	var oTextbox2 = new AutoSuggestControl2(document.getElementById("radiology"), new StateSuggestions2()); 
	var oTextbox3 = new AutoSuggestControl3(document.getElementById("services"), new StateSuggestions3()); 
	var oTextbox7 = new AutoSuggestControl7(document.getElementById("referal"), new StateSuggestions7()); 
...
<script type="text/javascript" src="js/autosuggestservices1a.js"></script>
<script type="text/javascript" src="js/autoservicescodesearch2.js"></script>

<?php include ("js/dropdownlist1scriptingreferal.php"); ?>
<script type="text/javascript" src="js/autocomplete_referal1a.js"></script>
<script type="text/javascript" src="js/autosuggestreferal1a.js"></script>
...
<table width="103%" border="0" cellspacing="0" cellpadding="2">
  ...
  <tr>
    ...
    <td width="97%" valign="top">
     <form name="form1" id="form1" method="post" action="consultationformreview.php" onSubmit="return process1();" onKeyDown="return disableEnterKey(event)">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="103%"><table   border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
            <tbody>
              <tr bgcolor="#011E6A">
                <td bgcolor="#CCCCCC" class="style2">Consultation</td>
				 <td bgcolor="#CCCCCC" class="style2">&nbsp;</td>
				 <td bgcolor="#CCCCCC" class="style2">&nbsp;</td>
				 <td bgcolor="#CCCCCC" class="style2">Location:&nbsp;<?php echo $locationname?></td>
                ...
                <input type="hidden" name="locationcode" id="locationcode" value="<?php echo $locationcode;?>">
				 ...
				 <td bgcolor="#CCCCCC" class="style2"><?php if($visitlimit != 0){ echo "Visit Limit"; } ?></td>
				 <td bgcolor="#CCCCCC" class="style2"><?php if($visitlimit != 0){ echo number_format($visitbalance,2,'.',','); } ?><input type="hidden" name="visitbalance" id="visitbalance" value="<?php echo $visitbalance; ?>">
				 </td>
              ...
              <!--<tr>
				...
				<tr>
                  ...
                  <tr>
				  <td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong>Fee Type</strong></td>
				  <td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><label><?php echo $feetypename; ?>
				    <input type="hidden" name="feetype" size="20" value="<?php echo $feetypename; ?>">
							  </label></td> 
                  </tr>
...
-->
				...
				<td colspan="6" align="left" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo ''; } else { echo '#AAFF00'; } ?>" class="bodytext3"><?php echo $errmsg;?>&nbsp;</td>
				 ...
				 <tr bgcolor="">
				  ...
				  <?php
					$query667 = "select * from master_consultationlist where patientcode = '$patientcode' and visitcode = '$visitcodenum' order by auto_number desc";
					...
					$templatedata = $res667['templatedata'];
				 ...
				 <td align="left" valign="middle"  bgcolor=""><label class="bodytext32">
				  ...
				  <strong>Intoxications</strong></span></td>
                   ...
                   <tr>
				   <td align="left" valign="top"  bgcolor=""><span class="bodytext32"><strong>MAF</strong></span></td>
				   <td align="left" valign="top"  bgcolor=""><label class="bodytext32">
	...
	/*				 
		 $query43="select * from billing_paylater where patientcode = '$patientcode' and billstatus = 'unpaid'";
	...
	while ($res3 = mysql_fetch_array($exec3))
				...
				/*
				  $querybilpharm="select sum(mcp.amount) as pharmrate,sum(prd.totalamount) as refundpharm from master_consultationpharm AS mcp LEFT JOIN  pharmacysalesreturn_details AS prd ON mcp.medicinecode=prd.itemcode AND mcp.patientvisitcode = prd.visitcode where mcp.patientcode = '$patientcode' and mcp.patientvisitcode='$viscode' and mcp.paymentstatus = 'completed' and mcp.approvalstatus <> '2' ";
					$execbilpharm=mysql_query($querybilpharm) or die(mysql_error());
					$resbilpharm=mysql_fetch_array($execbilpharm);
					$pharmrate=$resbilpharm['pharmrate'];
					$pharmrate1=$resbilpharm['refundpharm'];
					echo $pharmratevalue= $pharmrate-$pharmrate1;*/
					$pharmrefund=0;
				...
				//	$pharmratevalue= $pharmrate-$pharmrate1;
				 ...
				 {*/
					 $availablelimitshow=$availablelimit-$billedrate;
					 //$visitlimit1 = $visitlimit1 - $billedrate;
					 /*}*/
				   ?>
                   
                   <input size="10" type="hidden" name="availablelimit" id="availablelimit"  value="<?php echo $availablelimitshow; ?>" >
                   ...
                   <input size="8" type="text" readonly name="availablelimit100" id="availablelimit100"  value="<?php echo number_format($availablelimitshow, 2, '.', ','); ?>" disabled >
                   <?php } ?>
                   </td>
                   ...
                   <input size="8" type="text" readonly name="visitlimit12" id="visitlimit12"  value="<?php echo number_format($visitlimit1, 2, '.', ','); ?>" disabled >
                   </td>
                   <?php
				   }
				  ?>
				 ...
				 <tr onClick="collapsethis('consltid')">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="style2">
				    Consultation 
				   <span class="bodytext32">
				   <img src="images/plus1.gif" width="13" height="13" >				   </span>				   </td>
			      </tr>
				 <tr id="consltid">
				   <td colspan="7" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				 ...
				 <tr onClick="collapsethis('pressid')">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><strong>Prescription</strong> <span class="bodytext32"> <img src="images/plus1.gif" width="13" height="13" > </span></td>
			      </tr>
				 <tr id="pressid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				...
				?>
				 ...
				 <tr onClick="collapsethis('labid')">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><span class="style2">Lab <img src="images/plus1.gif" width="13" height="13"> </span></span></td>
			      </tr>
				 ...
				 <tr onClick="collapsethis('radid')">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><span class="style2">Radiology <img src="images/plus1.gif" width="13" height="13"> </span></span></td>
		        </tr>
				 <tr id="radid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				    <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
				 ...
				 <tr onClick="collapsethis('serid')">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><span class="style2">Services <img src="images/plus1.gif" width="13" height="13"> </span></span></td>
		        </tr>
				 <tr id="serid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				    <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
				...
				<input name="servicesname1hiddentextbox" id="servicesname1hiddentextbox" type="hidden">
				 ...
				 <tr onClick="collapsethis('reffid')">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><span class="style2">Internal Referral <img src="images/plus1.gif" width="13" height="13" > </span></span></td>
		        </tr>
				 <tr id="reffid">
				    <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				    <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
				  ...
				  <tr onClick="collapsethis('exreffid1')">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><span class="style2">External Referral <img src="images/plus1.gif" width="13" height="13" > </span></span></td>
		        </tr>
				<tr id="exreffid">
				 <td colspan="8" align="center" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
                 <!-- <a target="_blank" href="appointmentscheduleentry.php?patientcode=<?php echo $patientcode; ?>">Special Clinic Appointment</a> --></td>
				</tr>
				 <tr id="exreffid1">
				    <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
				    <table id="presid" width="417" border="0" cellspacing="1" cellpadding="1">
					 ...
					 <div id="insertrow4">					 </div></tr>
					  ...
					  <input type="hidden" name="referalcode" id="referalcode" value="">
				   	  <input name="referalname1hiddentextbox" id="referalname1hiddentextbox" type="hidden">
				 ...
				 <tr>
			        ...
			        <input type="text" id="total4" name="grandtotal1" readonly size="7"></td>
		        </tr> 
				<tr>
