...
$updatedatetime = date('Y-m-d H:i:s');
	...
	if ($res2 == 0)
	{
		...
		$query88 = "insert into master_consultation(consultation_id,patientcode,patientauto_number,patientname,patientvisitauto_number,patientvisitcode,recordstatus,recorddate,ipaddress,consultingdoctor,billtype,accountname,paymentstatus,sys,dia,pulse,temp,complaint,drugallergy,foodallergy,consultationtime) 
				values('$consultationid','$patientcode','$patientauto_number','$patientfullname','$patientvisit','$visitcode','completed','$currentdate','$ipaddress','$consultingdoctor','$billingtype','$accountname','pending','$bpsystolic','$bpdiastolic','$pulse','$celsius','$complaint','$drugallergy','$foodallergy','$currenttime')";
...
$maxanum = $labrefcode;
	...
	$categorylabquery1=mysql_query("insert into consultation_lab(consultationid,patientcode,patientname,patientvisitcode,labitemname,labitemrate,billtype,accountname,consultationdate,paymentstatus,refno,labsamplecoll,resultentry,labrefund)values('$consultationid','$patientcode','$patientfullname','$visitcode','$categorylabname','$categorylabrate','$billingtype','$accountname','$currentdate','$status','$labrefcode','pending','pending','norefund')") or die(mysql_error());
		...
		if($labname!="")
		{
		$labquery1=mysql_query("insert into consultation_lab(consultationid,patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,billtype,accountname,consultationdate,paymentstatus,refno,labsamplecoll,resultentry,labrefund,urgentstatus)values('$consultationid','$patientcode','$patientfullname','$visitcode','$labcode','$labname','$labrate','$billingtype','$accountname','$currentdate','$status','$labrefcode','pending','pending','norefund','$urgentstatus')") or die(mysql_error());
			}
		}
...
<script type="text/javascript" src="js/insertnewitem11.js"></script>
...
function btnDeleteClick33(delID10,vrate10)
{
	//alert ("Inside btnDeleteClick.");
	var newtotal30;
	//alert(vrate1);
	var varDeleteID10 = delID10;
	//alert(varDeleteID10);
	var fRet40; 
	fRet40 = confirm('Are You Sure Want To Delete This Entry?'); 
	//alert(fRet4); 
	if (fRet40 == false)
	{
		//alert ("Item Entry Not Deleted.");
	...
	var child10 = document.getElementById('idTR'+varDeleteID10);  //tr name
    var parent10 = document.getElementById('insertrow33'); // tbody name.
	document.getElementById ('insertrow33').removeChild(child10);
	
	var child10= document.getElementById('idTRaddtxt'+varDeleteID10);  //tr name
    var parent10= document.getElementById('insertrow33'); // tbody name.
	//alert (child);
	if (child10 != null) 
	{
	...
	var currenttotal30=document.getElementById('total1').value;
	//alert(currenttotal);
	newtotal30= currenttotal30-vrate10;
	
	//alert(newtotal3);
	
	document.getElementById('total1').value=newtotal30.toFixed(2);
	if(document.getElementById('total').value=='')
	{
	 totalamount11=0;
	//alert(totalamount11);
	...
	else
	{
	 ...
	 newgrandtotal3=parseInt(totalamount11)+parseInt(newtotal30)+parseInt(totalamount21)+parseInt(totalamount31)+parseInt(totalamount41);
				  ...
				  <tr id="labcategoryid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
					 ...
					 <div id="insertrow33">					 </div></tr>
					  ...
					  <td><label>
                       <input type="button" name="Add33" id="Add33" value="Add" onClick="return insertitem33()" class="button" style="border: 1px solid #001E6A">
                       </label></td>
					   </tr>
					    </table>	  </td> 
				  </tr>
