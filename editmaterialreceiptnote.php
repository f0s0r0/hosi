...
$itemstatus = '';
			...
			$query4 = "update materialreceiptnote_details set rate='$rate',quantity='$quantity',subtotal='$amount',free='$free',discountpercentage='$itemdiscountpercent',discountrupees='$itemdiscountrupees',totalamount='$amount',discountamount='$discountamount',username='$username',ipaddress='$ipaddress',entrydate='$billdate',itemtaxpercentage='$itemtaxpercent',itemtaxamount='$itemtaxamount',unit_abbreviation='$itemunitabb',batchnumber='$batchnumber',salesprice='$salesprice',expirydate='$expirydate',itemfreequantity='$free',itemtotalquantity='$quantity',
			packageanum='packageanum',packagename='$packagename',quantityperpackage='$quantityperpackage',allpackagetotalquantity='$allpackagetotalquantity',suppliername='$suppliername',suppliercode='$suppliercode',ponumber='$ponumber',supplierbillnumber='$supplierbillno',costprice='$costprice',location='$location',store='$store',coa='$coa',quotedprice='$quotedprice',priceperpack='$priceperpack' where billnumber='$billnumber' and ponumber='$ponumber' and itemcode='$itemcode'";
...
if (isset($_REQUEST["mrnnumber"])) { $mrnnumber = $_REQUEST["mrnnumber"]; } else { $mrnnumber = ""; }
//$cbfrmflag1 = $_POST['cbfrmflag1'];
	...
	$len1 = strlen($searchpo);
					...
					if($str1 == 'PO-')
					{
$query5 = "select * from master_rfqpurchaseorder where billnumber = '$searchpo' and recordstatus='generated' order by billnumber";
}
else
{
$query5 = "select * from master_rfqpurchaseorder where billnumber = '$searchpo' and recordstatus='autogenerated' order by billnumber";
}
...
while ($res5 = mysql_fetch_array($exec5))
{
	...
	$query445 = "select * from materialreceiptnote_details where ponumber='$billnum'";
	$exec445 = mysql_query($query445) or die(mysql_error());
	$res445 = mysql_fetch_array($exec445);
	$supplierbillnumber = $res445['supplierbillnumber'];
...
<?php
include("autocompletebuild_rfqpurchaseorder.php");
...
<script>
function totalcalc(varserialnumber,number)
{
...
var priceperpack = document.getElementById("priceperpack"+varserialnumber+"").value;
if(receivedqty != '')
{
...
if(parseFloat(receivedqty) > parseFloat(orderedqty))
{
...
var newbalanceqty = orderedqty - receivedqty;
document.getElementById("balqty"+varserialnumber+"").value = newbalanceqty;
var itemtotalamount = priceperpack * receivedqty;
document.getElementById("totalamount"+varserialnumber+"").value = itemtotalamount.toFixed(2);
if(receivedqty != '')
{
...
<?php include ("js/dropdownlistrfqpurchaseorder.php"); ?>
<script type="text/javascript" src="js/autocomplete_rfqpurchaseorder.js"></script>
...
-->
  <tr>
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td>
		 <form name="cbform1" method="post" action="editmaterialreceiptnote.php"> 
	  ...
	  <td>&nbsp;
			...
			<tr>
              <td bgcolor="#CCCCCC" class="bodytext3" colspan="8"><strong> Edit Material Receipt Note</strong></td>
			  ...
			  <tr>
				...
				<input name="po" id="po" value="<?php echo $billnum; ?>" style="border: 1px solid #001E6A" size="10" rsize="20" autocomplete="off" readonly="readonly"/>				</td>
             <td width="11%" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong>LPO Date </strong></td>
				...
				<input name="supplierbillno" id="supplierbillno" value="<?php echo $supplierbillnumber; ?>" style="border: 1px solid #001E6A" size="10" rsize="20" autocomplete="off" onKeyUp="return billnotransfer()"/>				</td>
      ...
      <tr>
        <td>
		<form action="editmaterialreceiptnote.php" method="post" name="form">
		 ...
		 <input type="hidden" name="supplierbillno1" id="supplierbillno1" value="<?php echo $supplierbillnumber; ?>">
             <tr>
                      ...
                      <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>Pkg.Size</strong></td>
					   ...
					   <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>QP/PK</strong></td>
			...
			$sno = 0;
			$totalamount=0;	
			$granditemtotalamount =0;
			$len = strlen($billnum);
					$str = preg_replace('/[^\\/\-a-z\s]/i', '', $billnum);
					if($str == 'PO-')
					{
			$query76 = "select * from master_rfqpurchaseorder where billnumber='$billnum' and recordstatus='generated' and goodsstatus=''";
			}
			else
			{
			$query76 = "select * from master_rfqpurchaseorder where billnumber='$billnum' and recordstatus='autogenerated' and goodsstatus=''";
			}
			...
			$packagequantity = $res761['packagequantity'];
			...
			$batchnumber = $res444['batchnumber'];
			$expirydate = $res444['expirydate'];
			$free = $res444['itemfreequantity'];
			$priceperpack = $res444['priceperpack'];
			$totalqty = $res444['allpackagetotalquantity'];
			$discountpercent = $res444['discountpercentage'];
			$itemtaxpercentage = $res444['itemtaxpercentage'];
			$itemtotalamount = $priceperpack * $receivedqty;
			$granditemtotalamount = $granditemtotalamount + $itemtotalamount;
...
?>
  <tr>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="receivedquantity[]" id="receivedquantity<?php echo $sno; ?>" size="6" onKeyUp="return totalcalc('<?php echo $sno; ?>','<?php echo $number; ?>');" class="bodytext21" autocomplete="off" value="<?php echo intval($receivedqty); ?>"></div></td>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="batch[]" id="batch<?php echo $sno; ?>" size="6" class="bodytext21" autocomplete="off" value="<?php echo $batchnumber; ?>"></div></td>
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="expirydate[]" id="expirydate<?php echo $sno; ?>" size="6" autocomplete="off" value="<?php echo $newexpirydate; ?>"></div></td>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="free[]" id="free<?php echo $sno; ?>" size="6" onKeyUp="return totalcalc1('<?php echo $sno; ?>');" class="bodytext21" autocomplete="off" value="<?php echo intval($free); ?>"></div></td>
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="totalquantity[]" id="totalquantity<?php echo $sno; ?>" size="6" class="bodytext21" readonly="readonly" value="<?php echo intval($totalqty); ?>"></div></td>
		<td class="bodytext31" valign="center"  align="left"><div align="right"><?php echo $rate; ?><input type="hidden" name="quotedprice[]" id="quotedprice" value="<?php echo $quotedprice; ?>" />
		...
		<td class="bodytext31" valign="center"  align="right"><div align="right"><input type="text" name="priceperpack[]" id="priceperpack<?php echo $sno; ?>" size="6" onKeyUp="return totalamount('<?php echo $sno; ?>','<?php echo $number; ?>');" class="bodytext21" autocomplete="off" value="<?php echo $priceperpack; ?>"></div></td>
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="discount[]" id="discount<?php echo $sno; ?>" size="6" onKeyUp="return totalamount5('<?php echo $sno; ?>','<?php echo $number; ?>');" class="bodytext21" autocomplete="off" value="<?php echo $discountpercent; ?>"></div></td>
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="tax[]" id="tax<?php echo $sno; ?>" size="6" onKeyUp="return totalamount20('<?php echo $sno; ?>','<?php echo $number; ?>');" class="bodytext21" autocomplete="off" value="<?php echo $itemtaxpercentage; ?>"></div></td>
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="totalamount[]" id="totalamount<?php echo $sno; ?>" size="6" class="bodytext21" readonly="readonly" value="<?php echo number_format($itemtotalamount,2,'.',','); ?>"></div></td>
	  ...
	  <table width="716">
     <tr>
	 <td width="20" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">Total Purchase Cost</td><input type="hidden" name="totalpurchaseamount1" id="totalpurchaseamount1" size="10" readonly="readonly" value="<?php echo $grandtotalamount; ?>">
	   <td width="111" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><input type="text" name="totalpurchaseamount" id="totalpurchaseamount" size="10" readonly="readonly" value="<?php echo number_format($granditemtotalamount,2,'.',','); ?>"></td>
      ...
      <tr>
		 ...
		 <td colspan="2" align="right" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">
		  <input type="hidden" name="frmflag2" value="frmflag2">
               <input name="Submit222" type="submit"  value="Save" class="button" style="border: 1px solid #001E6A" onClick="return funcsave('<?php echo $number; ?>')"/>		 </td>
      </tr>
	  </table>
      </td>
      </tr>
