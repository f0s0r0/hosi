...
foreach($_POST['radiology'] as $key => $value)
	...
	$extension = $_FILES['upload']['name'][$key];
	$extension1 = explode(".",trim($extension) );
	$patient = explode(' ',trim($patientname));
	$radiologyname1= str_replace(str_split('/'), ' ', $radiologyname);
	$radiologyname1 = str_replace( ' ', '_', $radiologyname1);
	$newname = $patientcode.'_'.$visitcode.'_'.$patient[0].'_'.$radiologyname1.'.'.$extension1[1];
	$uploadfile1 = $uploaddir.basename($newname);
	$uploadfile1 = str_replace( ' ', '_', $uploadfile1);
	move_uploaded_file($uploadfilename,$uploadfile1);
	$radiologyname1 = str_replace( '', '_', $radiologyname );
	$filename=basename($newname);
	
   // $extension = explode(".",$filename );
   //$extension1=$visitcode.'_'.$radiologyname.'.'.$extension[1];
   // $filename=$extension1;
...
if($radiologyname != "")
   {
       $query26="insert into resultentry_radiology(patientname,patientcode,patientvisitcode,recorddate,recordtime,itemcode,itemname,acknowledge,refund,templatedata,docnumber,account,billtype,username)values('$patientname','$patientcode',
   '$visitcode','$dateonly','$timeonly','$itemcode','$radiologyname','$status','$status1','$templatedata','$docnumber','$account','$billtype','$username')";
  ...
  header("location:radiologyentry1.php");
  exit();
}
...
function updateTextField()
 {
	...
	// alert(strText); 
    //document.getElementById('editor1').value= strText; 
	self.location='radiologyentry1.php?patientcode=<?php echo $patientcode; ?>&&visitcode=<?php echo $visitcode; ?>&&cat=' + strText ;
 }	
...
<form name="frmsales" id="frmsales" method="post" action="radiologyentry1.php" onKeyDown="return disableEnterKey(event)" enctype="multipart/form-data">
...
<tr></tr>
  <tr>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center">
			<select name="radiology[]" id="Class" onChange="updateTextField()">
				  <?php
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center">
			<input type="checkbox" name="ref[]" id="ref<?php echo $sno; ?>"value="<?php echo $itemcode; ?>" onClick="return checkboxcheck1('<?php echo $sno; ?>')"/></div></td>
			  ...
			  ?>
			   ...
			   <td colspan="3">
                    <textarea cols='50' rows='15' class="ckeditor" name="editor1" id="editor1"><strong><?php echo "Patient: ".$Patientname; ?> <?php echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Age: ".$patientage; ?>  <?php echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Gender: ".$patientgender; ?></strong><br/>
