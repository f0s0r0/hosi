    ...
    <td colspan="2" valign="top"><table width="99%" border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		<table width="1202" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
          <tbody>
            <tr bgcolor="#011E6A">
              <td colspan="4" bgcolor="#CCCCCC" class="bodytext3"><strong> Trial Balance CGS</strong></td>
              </tr>
             <tr>
		...
		{  
			...
			if($id == '01-1006')
			{
				...
				$querydr1in = "SELECT SUM(`amount`) as incomedr FROM `refund_paynowpharmacy` WHERE `billdate` BETWEEN '$ADate1' AND '$ADate2'
							   ...
							   UNION ALL SELECT SUM(`servicesitemrate`) as incomedr FROM `refund_paylaterservices` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							   UNION ALL SELECT SUM(`servicesitemrate`) as incomedr FROM `refund_paynowservices` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							   UNION ALL SELECT SUM(`amount`) as incomedr FROM `ip_creditnotebrief` WHERE consultationdate BETWEEN '$ADate1' AND '$ADate2' and `accountcode` <> ''
							   UNION ALL SELECT SUM(-1*`amount`) as incomedr FROM `ip_debitnotebrief` WHERE consultationdate BETWEEN '$ADate1' AND '$ADate2' and `accountcode` <> ''";
				...
				$accountbank = array_sum($crresult) - array_sum($drresult);
			}
			...
			else
			{
				$i = 0;
				$drresult = array();
				$querydr1bnk = "SELECT SUM(transactionamount) as bankcredit FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `docno` LIKE '%IPCr%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'paylatercredit'
								UNION ALL SELECT SUM(-1*transactionamount) as bankcredit FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `billnumber` LIKE '%IPDr%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
								UNION ALL SELECT SUM(transactionamount) as bankcredit FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `docno` LIKE 'Cr.N%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'paylatercredit'
								UNION ALL SELECT SUM(`amount`) as bankcredit FROM `paylaterpharmareturns` WHERE billdate BETWEEN '$ADate1' AND '$ADate2' AND `billnumber` IN (SELECT b.`docno` FROM `master_transactionpaylater` as a JOIN `master_transactionpaylater` as b ON (a.`visitcode` = b.`visitcode`) WHERE b.`accountnameid` = '$id' and a.`transactiontype` = 'finalize' and b.`transactiontype` = 'pharmacycredit' and b.`auto_number` > a.`auto_number`)
				...
				$accountbank = 0 - array_sum($crresult) - array_sum($drresult);		
