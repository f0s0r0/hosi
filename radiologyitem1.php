	...
	for($i=1; $i<=$loccountloop; $i++)
{
	...
	if ($length1<25 && $length2<255)
	{
		...
		$query2 = "select * from master_radiology where itemcode = '$itemcode' and locationcode='".$loccodeget."'";// or itemname = '$itemname'";
		$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
		$res2 = mysql_num_rows($exec2);
		if ($res2 == 0)
		{
			$query1 = "insert into master_radiology (itemcode, itemname, categoryname,  rateperunit, expiryperiod, ipaddress, pkg, updatetime, description,  referencevalue,ipmarkup,locationcode) 
			values ('$itemcode', '$itemname', '$categoryname',  '".$locrateget."', '$expiryperiod',  '$ipaddress', '".$pkg."', '$updatedatetime','manual',  '$referencevalue','$ipmarkup','".$loccodeget."')";
			...
			while ($res25 = mysql_fetch_array($exec25))
			{
				...
				$query21 = "insert into `$templatename` (itemcode, itemname, categoryname,  rateperunit, expiryperiod, ipaddress, pkg, updatetime, description,  referencevalue,ipmarkup,locationcode) 
			values ('$itemcode', '$itemname', '$categoryname',  '".$locrateget."', '$expiryperiod',  '$ipaddress', '".$pkg."', '$updatedatetime','manual',  '$referencevalue','$ipmarkup','".$loccodeget."')";
		...
		{
			$errmsg = "Failed. radiology Item Code Already Exists.";
			$bgcolorcode = 'failed';
		}
	}
	else
...
<table width="101%" border="0" cellspacing="0" cellpadding="2">
  ...
  <tr>
    ...
    <td width="97%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="860"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebackgroundcolor1">
            <tr>
              <td><form name="form1" id="form1" method="post" action="radiologyitem1.php" onSubmit="return additem1process1()">
                  <table width="900" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                    <tbody>
                      <tr bgcolor="#011E6A">
                        <td colspan="4" bgcolor="#CCCCCC" class="bodytext3"><strong>Radiology Item Master - Add New </strong></td>
                      </tr>
					  <?php if ($st==1)
					  {?>
					  <tr>
      ...
      <tr>
	   <td width="9%" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3">&nbsp;</td>
        <td width="10px" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3" ><input type="checkbox" name="lcheck<?php echo $incr;?>" id="lcheck<?php echo $incr;?>" style="float:left" value="<?php echo $locationcode;?>"><input type="hidden" name="checklocval" value=<?php echo $locationcode;?>"">
      ...
      <?php }?><input type="hidden" name="locationcount" value="<?php echo  $incr;?>">
                ...
                <table width="900" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                    <tbody>
                      <tr bgcolor="#011E6A">
                        <td colspan="4" bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><strong>Radiology Item Master - Existing List - Latest 100 radiology Items </strong></span></td>
						...
						if($searchflag1 != 'searchflag1'){
							...
							*/
							$query111 = "select * from master_radiology where status <> 'deleted' order by auto_number desc";
	  ...
	  if ($searchflag1 == 'searchflag1')
	  {
	    ...
	    $query1 = "select * from master_radiology where itemname like '%$search1%' or categoryname like '%$search1%' and status <> 'deleted' group by itemcode order by auto_number desc LIMIT 100";
		$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
		while ($res1 = mysql_fetch_array($exec1))
		{
	...
	{
	$query1 = "select * from master_radiology where status <> 'deleted'  group by itemcode  order by auto_number desc LIMIT $start , $limit";
		$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
		while ($res1 = mysql_fetch_array($exec1))
		{
		$itemcode = $res1["itemcode"];
		...
		?>
                ...
                <table width="900" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                    <tbody>
                      <tr bgcolor="#011E6A">
                        <td colspan="11" bgcolor="#CCCCCC" class="bodytext3"><strong>Radiology Item Master - Deleted </strong></td>
                      </tr>
                      <tr bgcolor="#011E6A">
	  ...
	  if ($searchflag2 == 'searchflag2')
	  {
	    ...
	    $query1 = "select * from master_radiology where itemname like '%$search2%' or categoryname like '%$search1%' and status = 'deleted' order by auto_number desc LIMIT 100";
		$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
		while ($res1 = mysql_fetch_array($exec1))
		{
		$itemcode = $res1["itemcode"];
	...
	else
	{
	    ...
	    $query1 = "select * from master_radiology where status = 'deleted' order by auto_number desc LIMIT 100";
		$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
		while ($res1 = mysql_fetch_array($exec1))
		{
		$itemcode = $res1["itemcode"];
