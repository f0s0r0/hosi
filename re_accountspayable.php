...
<tr>
<td colspan="12" align="center" valign="middle"><strong>Purchase Reconcile</strong></td>
</tr>
...
<td align="left"><strong>Supplier Code</strong></td>
<td align="right"><strong>Purchases (Dr)</strong></td>
<td align="right"><strong>Assets (Dr)</strong></td>
<td align="right"><strong>Expense (Dr)</strong></td>
...
<?php 
$query8 = "SELECT billnumber,transactionamount,suppliername,suppliercode,transactiondate FROM `master_transactionpharmacy` WHERE `billnumber` NOT LIKE 'SOP%' AND `transactiontype` = 'PURCHASE' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' order by auto_number";
...
while($res8 = mysql_fetch_array($exec8))
{
	...
	$query1 = "SELECT SUM(`totalamount`) as stock FROM `purchase_details` WHERE `billnumber` = '$billnumber' and entrydate = '$transactiondate'";
	$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
	$res1 = mysql_fetch_array($exec1);
	$stock = $res1['stock'];
	
	$query2 = "SELECT SUM(`totalamount`) as asset FROM `assetpurchase_details` WHERE `billnumber` = '$billnumber' and entrydate = '$transactiondate'";
	$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
	$res2 = mysql_fetch_array($exec2);
	$asset = $res2['asset'];
	
	$query3 = "SELECT SUM(`totalamount`) as expense FROM `expensepurchase_details` WHERE `billnumber` = '$billnumber' and entrydate = '$transactiondate'";
	$exec3 = mysql_query($query3) or die ("Error in Query3".mysql_error());
	$res3 = mysql_fetch_array($exec3);
	$expense = $res3['expense'];
	
	$total = $stock + $asset + $expense;
	$amount2 = $amount2 + $total;
	$diff = $purchaseamount - $total;
...
<td align="right" valign="middle"><?= number_format($stock,2); ?></td>
<td align="right" valign="middle"><?= number_format($asset,2); ?></td>
<td align="right" valign="middle"><?= number_format($expense,2); ?></td>
...
<table>
       ...
       <tr>
        <td valign="top"><table width="600" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			{	
				...
				$query2 = "select accountssub,auto_number,tbinclude,tbledgerview from master_accountssub where (auto_number IN ('29','16') or accountsmain = '4') AND accountsmain = '$parentid' and recordstatus <> 'deleted' order by accountssub";
				$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
 ...
 <td width="54%" valign="top"><table width="600" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			{	
				...
				if (isset($_REQUEST["ADate2"])) { $ADate2 = $_REQUEST["ADate2"]; } else { $ADate2 = ""; }
				if (isset($_REQUEST["location"])) { $location = $_REQUEST["location"]; } else { $location = ""; }
				$query2 = "select accountssub, auto_number,tbinclude,tbledgerview from master_accountssub where auto_number IN ('23','24') and accountsmain = '$parentid' and recordstatus <> 'deleted' order by accountssub";
				$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
				while($res2 = mysql_fetch_array($exec2))
				{  
					$accountsmain2 = $res2['accountssub'];
	...
	if($parentid != '')
	{
		/*if($parentid == '16')
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(`totalamount`) as stock FROM `purchase_details` WHERE `entrydate` BETWEEN '$ADate1' AND '$ADate2' AND `suppliername` <> 'OPENINGSTOCK' AND `expensecode` = '$id' AND `recordstatus` <> 'deleted'";
		...
		if($parentid == '29')
		{
			...
			{  
				...
				if($id == '03-1001-01')
				{
					$querydr1bnk = "SELECT SUM(`totalamount`) as stock FROM `purchase_details` WHERE `entrydate` BETWEEN '$ADate1' AND '$ADate2' AND `suppliername` <> 'OPENINGSTOCK' AND `expensecode` = '' AND `recordstatus` <> 'deleted'";
					$execdr1 = mysql_query($querydr1bnk) or die ("Error in querydr1bnk".mysql_error());
					while($resdr1 = mysql_fetch_array($execdr1))
					{
					$i = $i+1;
		...
		else if($parentid == '23' || $parentid == '24')
		{
			$query267 = "select accountname,auto_number,id from master_accountname where accountssub = '$parentid'";
			$exec267 = mysql_query($query267) or die ("Error in Query267".mysql_error());
			while($res267 = mysql_fetch_array($exec267))
			{  
				...
				$i = 0;
				$crresult = array();
				$querycr1bnk = "SELECT SUM(`transactionamount`) as payables FROM `master_transactionpharmacy` WHERE `suppliercode` = '$id' AND `transactiontype` = 'PURCHASE' AND particulars <> '' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'";
				$execcr1 = mysql_query($querycr1bnk) or die ("Error in querycr1bnk".mysql_error());
				while($rescr1 = mysql_fetch_array($execcr1))
				{
				$i = $i+1;
				$crresult[$i] = $rescr1['payables'];
				}
		...
		else
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(`totalamount`) as expense FROM `expensepurchase_details` WHERE `expensecode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' and recordstatus <> 'deleted'
								UNION ALL SELECT SUM(`totalamount`) as expense FROM `purchase_details` WHERE `expensecode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' AND `recordstatus` <> 'deleted'";
				$execdr1 = mysql_query($querydr1bnk) or die ("Error in querydr1bnk".mysql_error());
				while($resdr1 = mysql_fetch_array($execdr1))
				{
				$i = $i+1;
				$drresult[$i] = $resdr1['expense'];
				}
