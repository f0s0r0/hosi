...
if ($frm1submit1 == 'frm1submit1')
{
	...
	if($num386 == 0)
	{
		...
		$query38611="select * from billing_paynow where visitcode='$visitcode' and consultationid='$consultationid'";
		$exec38611=mysql_query($query38611) or die(mysql_error());
		$num38611=mysql_num_rows($exec38611);
		
		
		if($num38611!=0 && ($ambcount > 0 || $ambcount1 > 0) )
		{
			
		$paynowbillprefixn = 'AMHM-';
		$paynowbillprefix1=strlen($paynowbillprefixn);
 	
	
		$query91="select * from billing_paynow where consultationid like '%AMHM%'  ";
		$exec91 = mysql_query($query91) or die(mysql_error());
		$num91 = mysql_num_rows($exec91);
		if($num91 ==0)
		{
		$billnumbernew = $paynowbillprefixn.'1';
		}
		else
		{
		$querycon="select * from billing_paynow where consultationid like '%AMHM%' order by auto_number desc";
		$execcon=mysql_query($querycon) or die(mysql_error());
		$numcon=mysql_num_rows($execcon);
		$rescon=mysql_fetch_array($execcon);
		$billnumbernewval1=$rescon['consultationid'];
			
		$billdigit=strlen($billnumbernewval1);
		$billnumbernew = substr($billnumbernewval1,$paynowbillprefix1, $billdigit);
		//echo $billnumbercode;
		$billnumbernew = intval($billnumbernew);
		$billnumbernew = $billnumbernew + 1;
	
		$maxanum = $billnumbernew;
		
		
		$billnumbernew = 'AMHM-' .$maxanum;
		
		
		}
			
			$consultationid=$billnumbernew;
		}
		...
		if($ambcount>0)
		{
			...
			foreach($_POST['accountnameamb'] as $key =>$value)
			{ 
			
				$amdocno=$_REQUEST['amdocno'][$key];
				$accountname=$_REQUEST['accountnameamb'][$key];
				...
				//$referalquery133=mysql_query("insert into billing_opambulance(docno,patientcode,patientname,visitcode,accountname,description,quantity,rate,amount,locationname,locationcode,recordtime,recordstatus,ipaddress,recorddate,username,billnumber)values('".$amdocno."','$patientcode','$patientname','$visitcode','$accountname','".$description."','".$quantity."','".$rate."','".$amount."','$locationname','$locationcode','".$timeonly."','paid','".$ipaddress."','".$dateonly."','".$username."','$billnumber')") or die(mysql_error());
				}
			...
			if($ambcount1>0)
		{
			...
			foreach($_POST['ambulancecount1'] as $key =>$value)
			{ 
				...
				//$referalquery1232=mysql_query("insert into billing_homecare(docno,patientcode,patientname,visitcode,accountname,description,quantity,rate,amount,locationname,locationcode,recordtime,recordstatus,ipaddress,recorddate,username,billnumber)values('".$amdocno1."','$patientcode','$patientname','$visitcode','$accountname1','".$description1."','".$quantity1."','".$rate1."','".$amount1."','$locationname','$locationcode','".$timeonly."','paid','".$ipaddress."','".$dateonly."','".$username."','$billnumber')") or die(mysql_error());
				}
				
		mysql_query("update homecare set paymentstatus='completed' where patientvisitcode='$visitcode'");
			}
			//ends here 
	$consid = substr($consultationid,0, 3); 
	
		if($consid =='AMH')
		{
		
	 mysql_query("update master_billing set conscheck='AMHO' where visitcode='$visitcode'"); 
		}
	...
	if($num3861 == 0)
	{
		...
		if($consid =='AMH')
		{
		mysql_query("update master_billing set conscheck='AMHO' where patientvisitcode='$visitcode'");
		}
	*/
		if($consid !='AMH')
		{	
		    $query29 = "select * from master_consultationpharm where patientvisitcode='$visitcode' and patientcode='$patientcode' and approvalstatus !='0'";
		...
		mysql_query("update master_consultationpharm set pharmacybill='completed' where patientvisitcode='$visitcode' and approvalstatus !='0'"); 
		}
	...
	$query34=mysql_query("update master_consultationpharm set billing='completed' where medicinename='$pharmname' and patientvisitcode='$visitcode' and approvalstatus !='0' ") or die(mysql_error());
	$query38=mysql_query("update master_consultationpharmissue set paymentstatus='completed' where medicinename='$pharmname' and patientvisitcode='$visitcode' and approvalstatus !='0'");
	}
	...
	$query27=mysql_query("update consultation_lab set paymentstatus='completed' where patientvisitcode='$visitcode' and approvalstatus !='0'");
...
$query28=mysql_query("update consultation_radiology set paymentstatus='completed' where patientvisitcode='$visitcode' and approvalstatus !='0'") or die(mysql_query());
   ...
   $query24=mysql_query("insert into billing_services(billnumber,patientcode,patientname,accountname,subtotal,billdate,totalamountafterdis,grandtotal,paymentmode,locationname,locationcode)values('$billnumber','$patientcode','$patientname','$accountname','$subtotal','$billingdatetime','$totalamountafterdis','$patientbillamount','$patientpaymentmode','$locationname','$locationcode')") or die(mysql_error());
	...
	$query29=mysql_query("update consultation_services set paymentstatus='completed' where patientvisitcode='$visitcode' and approvalstatus !='0'");
	...
	$query30=mysql_query("update consultation_departmentreferal set paymentstatus='completed' where patientvisitcode='$visitcode'");
...
foreach($_POST['medicinename'] as $key=>$value)
		{	
			...
			$rate1=$res77[$locationcode."_rateperunit"];
			...
			$resofquerygetdis=mysql_fetch_array($execquerygetdis);
			 $phardiscount=intval($resofquerygetdis["pharmacydisc"]);
			  ...
			  $pharate2=($rate1*$phardiscount)/100;
	...
	$exec32 = mysql_query($querydisp) or die ("Error in Querydisp".mysql_error());
			...
			if ($medicinename != "")// && $dose != "" && $frequency != "" && $days != "" && $quantity != "" && $instructions != "" && $rate != "" && $amount != "")
			{
		       ...
		       // $query2 = "insert into billing_paynowpharmacy(patientcode,patientname,patientvisitcode,medicinename,quantity,rate,amount,billdate,ipaddress,accountname,paymentstatus,medicinecode,billnumber,username,pharmacycoa,locationname,locationcode,discount) 
				//values('$patientcode','$patientname','$visitcode','$medicinename','$quantity','$rate','$amount','$billdate','$ipaddress','$accountname','unpaid','$medicinecode','$billnumber','$username','$pharmacycoa','$locationname','$locationcode','$pharate2')";
				//$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
			
							
			}
			
			$query54 = "update master_consultationpharm set excludebill='completed' where medicinecode='$medicinecode' and patientvisitcode='$visitcode' and excludestatus='excluded' and approvalstatus !='0'";
			$exec54 = mysql_query($query54) or die(mysql_error());
			
			$query55 = "update master_consultationpharmissue set excludebill='completed' where medicinecode='$medicinecode' and patientvisitcode='$visitcode' and excludestatus='excluded' and approvalstatus !='0'";
		...
		{	
				...
				$query551 = "insert into billing_paynowpharmacy(patientcode,patientname,patientvisitcode,medicinename,quantity,rate,amount,billdate,ipaddress,accountname,paymentstatus,medicinecode,billnumber,username,pharmacycoa,locationname,locationcode,transactionmode,cashamount,cashcode) 
				values('$patientcode','$patientname','$visitcode','Dispensing Fee','1','$dispensingfee','$dispensingfee','$billdate','$ipaddress','$accountname','unpaid','dispensingfee','$billnumber','$username','$pharmacycoa','$locationname','$locationcode','CASH','$dispensingfee','$cashcode')";
				$exec551 = mysql_query($query551) or die ("Error in Query551".mysql_error());
			...
			if($paymentmode != 'SPLIT')
				{
					 ...
					 $labquery1=mysql_query("insert into billing_paynowlab(patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,accountname,billdate,paymentstatus,billnumber,username,labcoa,locationname,locationcode,transactionmode,cashamount,cashcode)values('$patientcode','$patientname','$visitcode','$labcode','$labname','$labrate','$accountname','$billdate','unpaid','$billnumber','$username','$labcoa','$locationname','$locationcode','CASH','$labrate','$cashcode')") or die(mysql_error());
					 ...
					 $labquery1=mysql_query("insert into billing_paynowlab(patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,accountname,billdate,paymentstatus,billnumber,username,labcoa,locationname,locationcode,transactionmode,chequeamount,bankcode)values('$patientcode','$patientname','$visitcode','$labcode','$labname','$labrate','$accountname','$billdate','unpaid','$billnumber','$username','$labcoa','$locationname','$locationcode','CHEQUE','$labrate','$chequecode')") or die(mysql_error());
					 ...
					 $labquery1=mysql_query("insert into billing_paynowlab(patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,accountname,billdate,paymentstatus,billnumber,username,labcoa,locationname,locationcode,transactionmode,onlineamount,bankcode)values('$patientcode','$patientname','$visitcode','$labcode','$labname','$labrate','$accountname','$billdate','unpaid','$billnumber','$username','$labcoa','$locationname','$locationcode','ONLINE','$labrate','$chequecode')") or die(mysql_error());
					 ...
					 $labquery1=mysql_query("insert into billing_paynowlab(patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,accountname,billdate,paymentstatus,billnumber,username,labcoa,locationname,locationcode,transactionmode,creditamount,mpesacode)values('$patientcode','$patientname','$visitcode','$labcode','$labname','$labrate','$accountname','$billdate','unpaid','$billnumber','$username','$labcoa','$locationname','$locationcode','MPESA','$labrate','$mpesacode')") or die(mysql_error());
					 ...
					 $labquery1=mysql_query("insert into billing_paynowlab(patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,accountname,billdate,paymentstatus,billnumber,username,labcoa,locationname,locationcode,transactionmode,cardamount,bankcode)values('$patientcode','$patientname','$visitcode','$labcode','$labname','$labrate','$accountname','$billdate','unpaid','$billnumber','$username','$labcoa','$locationname','$locationcode','CREDIT CARD','$labrate','$chequecode')") or die(mysql_error());
				 ...
				 else
				 {
					...
					$labquery124 =mysql_query("insert into billing_paynowlab(patientcode,patientname,patientvisitcode,labitemcode,labitemname,labitemrate,accountname,billdate,paymentstatus,billnumber,username,labcoa,locationname,locationcode,transactionmode,cashamount,onlineamount,chequeamount,cardamount,creditamount,bankcode,cashcode,mpesacode)values('$patientcode','$patientname','$visitcode','$labcode','$labname','$labrate','$accountname','$billdate','unpaid','$billnumber','$username','$labcoa','$locationname','$locationcode','SPLIT','$labrate','0.00','0.00','0.00','0.00','$chequecode','$cashcode','$mpesacode')") or die(mysql_error());
			...
			if($paymentmode != 'SPLIT')
				{
					 ...
					 $radiologyquery1=mysql_query("insert into billing_paynowradiology(patientcode,patientname,patientvisitcode,radiologyitemcode,radiologyitemname,radiologyitemrate,accountname,billdate,paymentstatus,billnumber,username,radiologycoa,locationname,locationcode,transactionmode,cashamount,cashcode)values('$patientcode','$patientname','$visitcode','$radiologycode','$pairs','$pairs1','$accountname','$billdate','unpaid','$billnumber','$username','$radiologycoa','$locationname','$locationcode','CASH','$pairs1','$cashcode')") or die(mysql_error());
					 ...
					 $radiologyquery1=mysql_query("insert into billing_paynowradiology(patientcode,patientname,patientvisitcode,radiologyitemcode,radiologyitemname,radiologyitemrate,accountname,billdate,paymentstatus,billnumber,username,radiologycoa,locationname,locationcode,transactionmode,chequeamount,bankcode)values('$patientcode','$patientname','$visitcode','$radiologycode','$pairs','$pairs1','$accountname','$billdate','unpaid','$billnumber','$username','$radiologycoa','$locationname','$locationcode','CHEQUE','$pairs1','$chequecode')") or die(mysql_error());
					 ...
					 $radiologyquery1=mysql_query("insert into billing_paynowradiology(patientcode,patientname,patientvisitcode,radiologyitemcode,radiologyitemname,radiologyitemrate,accountname,billdate,paymentstatus,billnumber,username,radiologycoa,locationname,locationcode,transactionmode,onlineamount,bankcode)values('$patientcode','$patientname','$visitcode','$radiologycode','$pairs','$pairs1','$accountname','$billdate','unpaid','$billnumber','$username','$radiologycoa','$locationname','$locationcode','ONLINE','$pairs1','$chequecode')") or die(mysql_error());
					 ...
					 $radiologyquery1=mysql_query("insert into billing_paynowradiology(patientcode,patientname,patientvisitcode,radiologyitemcode,radiologyitemname,radiologyitemrate,accountname,billdate,paymentstatus,billnumber,username,radiologycoa,locationname,locationcode,transactionmode,creditamount,mpesacode)values('$patientcode','$patientname','$visitcode','$radiologycode','$pairs','$pairs1','$accountname','$billdate','unpaid','$billnumber','$username','$radiologycoa','$locationname','$locationcode','MPESA','$pairs1','$mpesacode')") or die(mysql_error());
					 ...
					 $radiologyquery1=mysql_query("insert into billing_paynowradiology(patientcode,patientname,patientvisitcode,radiologyitemcode,radiologyitemname,radiologyitemrate,accountname,billdate,paymentstatus,billnumber,username,radiologycoa,locationname,locationcode,transactionmode,cardamount,bankcode)values('$patientcode','$patientname','$visitcode','$radiologycode','$pairs','$pairs1','$accountname','$billdate','unpaid','$billnumber','$username','$radiologycoa','$locationname','$locationcode','CREDIT CARD','$pairs1','$chequecode')") or die(mysql_error());
				 ...
				 else
				 {
					...
					$radiologyquery1=mysql_query("insert into billing_paynowradiology(patientcode,patientname,patientvisitcode,radiologyitemcode,radiologyitemname,radiologyitemrate,accountname,billdate,paymentstatus,billnumber,username,radiologycoa,locationname,locationcode,transactionmode,cashamount,onlineamount,chequeamount,cardamount,creditamount,bankcode,cashcode,mpesacode)values('$patientcode','$patientname','$visitcode','$radiologycode','$pairs','$pairs1','$accountname','$billdate','unpaid','$billnumber','$username','$radiologycoa','$locationname','$locationcode','SPLIT','$pairs1','0.00','0.00','0.00','0.00','$chequecode','$cashcode','$mpesacode')") or die(mysql_error());
			...
			if($paymentmode != 'SPLIT')
				{
					 ...
					 $servicesquery1=mysql_query("insert into billing_paynowservices(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,accountname,billdate,paymentstatus,billnumber,username,servicecoa,locationname,locationcode,serviceqty,amount,transactionmode,cashamount,cashcode)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$accountname','$billdate','unpaid','$billnumber','$username','$servicecoa','$locationname','$locationcode','".$quantityser."','".$seramount."','CASH','$servicesrate','$cashcode')") or die(mysql_error());
					 ...
					 $servicesquery1=mysql_query("insert into billing_paynowservices(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,accountname,billdate,paymentstatus,billnumber,username,servicecoa,locationname,locationcode,serviceqty,amount,transactionmode,chequeamount,bankcode)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$accountname','$billdate','unpaid','$billnumber','$username','$servicecoa','$locationname','$locationcode','".$quantityser."','".$seramount."','CHEQUE','$servicesrate','$chequecode')") or die(mysql_error());
					 ...
					 $servicesquery1=mysql_query("insert into billing_paynowservices(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,accountname,billdate,paymentstatus,billnumber,username,servicecoa,locationname,locationcode,serviceqty,amount,transactionmode,onlineamount,bankcode)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$accountname','$billdate','unpaid','$billnumber','$username','$servicecoa','$locationname','$locationcode','".$quantityser."','".$seramount."','ONLINE','$servicesrate','$chequecode')") or die(mysql_error());
					 ...
					 $servicesquery1=mysql_query("insert into billing_paynowservices(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,accountname,billdate,paymentstatus,billnumber,username,servicecoa,locationname,locationcode,serviceqty,amount,transactionmode,creditamount,mpesacode)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$accountname','$billdate','unpaid','$billnumber','$username','$servicecoa','$locationname','$locationcode','".$quantityser."','".$seramount."','MPESA','$servicesrate','$mpesacode')") or die(mysql_error());
					 ...
					 $servicesquery1=mysql_query("insert into billing_paynowservices(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,accountname,billdate,paymentstatus,billnumber,username,servicecoa,locationname,locationcode,serviceqty,amount,transactionmode,cardamount,bankcode)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$accountname','$billdate','unpaid','$billnumber','$username','$servicecoa','$locationname','$locationcode','".$quantityser."','".$seramount."','CREDIT CARD','$servicesrate','$chequecode')") or die(mysql_error());
				 ...
				 else
				 {
					...
					$servicesquery1=mysql_query("insert into billing_paynowservices(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,accountname,billdate,paymentstatus,billnumber,username,servicecoa,locationname,locationcode,serviceqty,amount,transactionmode,cashamount,onlineamount,chequeamount,cardamount,creditamount,bankcode,cashcode,mpesacode)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$accountname','$billdate','unpaid','$billnumber','$username','$servicecoa','$locationname','$locationcode','".$quantityser."','".$seramount."','SPLIT','$servicesrate','0.00','0.00','0.00','0.00','$chequecode','$cashcode','$mpesacode')") or die(mysql_error());											
				 }
			...
			if($paymentmode != 'SPLIT')
				{
					 ...
					 $referalquery2=mysql_query("insert into billing_paynowreferal(patientcode,patientname,patientvisitcode,referalcode,referalname,referalrate,accountname,billdate,paymentstatus,billnumber,username,referalcoa,locationname,locationcode,transactionmode,chequeamount,bankcode)values('$patientcode','$patientname','$visitcode','$referalcode','$pairvar2','$pairvar3','$accountname','$billdate','unpaid','$billnumber','$username','$referalcoa','$locationname','$locationcode','CHEQUE','$pairvar3','$chequecode')") or die(mysql_error());
					 ...
					 $referalquery2=mysql_query("insert into billing_paynowreferal(patientcode,patientname,patientvisitcode,referalcode,referalname,referalrate,accountname,billdate,paymentstatus,billnumber,username,referalcoa,locationname,locationcode,transactionmode,onlineamount,bankcode)values('$patientcode','$patientname','$visitcode','$referalcode','$pairvar2','$pairvar3','$accountname','$billdate','unpaid','$billnumber','$username','$referalcoa','$locationname','$locationcode','ONLINE','$pairvar3','$chequecode')") or die(mysql_error());
					 ...
					 $referalquery2=mysql_query("insert into billing_paynowreferal(patientcode,patientname,patientvisitcode,referalcode,referalname,referalrate,accountname,billdate,paymentstatus,billnumber,username,referalcoa,locationname,locationcode,transactionmode,creditamount,mpesacode)values('$patientcode','$patientname','$visitcode','$referalcode','$pairvar2','$pairvar3','$accountname','$billdate','unpaid','$billnumber','$username','$referalcoa','$locationname','$locationcode','MPESA','$pairvar3','$mpesacode')") or die(mysql_error());
					 ...
					 $referalquery2=mysql_query("insert into billing_paynowreferal(patientcode,patientname,patientvisitcode,referalcode,referalname,referalrate,accountname,billdate,paymentstatus,billnumber,username,referalcoa,locationname,locationcode,transactionmode,cardamount,bankcode)values('$patientcode','$patientname','$visitcode','$referalcode','$pairvar2','$pairvar3','$accountname','$billdate','unpaid','$billnumber','$username','$referalcoa','$locationname','$locationcode','CREDIT CARD','$pairvar3','$chequecode')") or die(mysql_error());
	...
	$transactiontype = 'PAYMENT';
		...
		header("location:patientbillingstatus.php?paynowpatientcode=$patientcode&&paynowbillnumber=$billnumber");
		exit;
		}
...
$querylab2=mysql_query("select locationcode from master_visitentry where visitcode='$visitcode'");
$execlab2=mysql_fetch_array($querylab2);
...
<form name="form1" id="frmsales" method="post" action="billing_paynow.php" onKeyDown="return disableEnterKey(event)" onSubmit="return funcSaveBill1()">
           ...
           <tr bgcolor="#011E6A">
			...
			<td width="4%"  align="left" valign="center" 
			  ...
			  ?>
			   ...
			   $totalpharm=0;
			   $pharmno=0;
			$query23 = "select * from master_consultationpharm where patientvisitcode='$visitcode' and patientcode='$patientcode' and pharmacybill='pending' and medicineissue='pending' and billing='' and approvalstatus !='0'";
			...
			$resofquerygetdis=mysql_fetch_array($execquerygetdis);
			 $phardiscount=intval($resofquerygetdis["pharmacydisc"]);
			  ...
			  $pharate2=($pharate1*$phardiscount)/100;
			
			$pharate=$pharate1-$pharate2;
...
if ($showcolor == 0)
			...
			$totallab=$totallab+$dispensingamount; 
			  ?>
			...
			?>
			 ...
			 <td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $sno = $sno + 1; ?></div></td>
			  <td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $refdate; ?></div></td>
			  <input type="hidden" name="referalnameamb" value="<?php echo $refname; ?>">
			 <td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $refref; ?></div></td>
			 <td class="bodytext31" valign="center"  align="left"><div align="left"><?php echo $refname; ?></div></td>
			  <input name="referalamb[]" type="hidden" id="referal" size="69" value="<?php echo $refname; ?>">
			 <input name="rate4amb[]" type="hidden" id="rate4" readonly size="8" value="<?php echo $refrate; ?>">
             <input type="hidden" name="ambulancecount[]" value="<?php echo $sno-1;?>">
              <input name="accountnameamb[]" type="hidden" id="accountname" readonly size="8" value="<?php echo $accountname; ?>">
			...
			?>
			 ...
			 <td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $sno = $sno + 1; ?></div></td>
			  <td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $refdate; ?></div></td>
			  <input type="hidden" name="referalnamehom" value="<?php echo $refname; ?>">
			 <td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $refref; ?></div></td>
			 <td class="bodytext31" valign="center"  align="left"><div align="left"><?php echo $refname; ?></div></td>
			  <input name="referalhom[]" type="hidden" id="referal" size="69" value="<?php echo $refname; ?>">
			 <input name="rate4hom[]" type="hidden" id="rate4" readonly size="8" value="<?php echo $refrate; ?>">
...
bgcolor="#F3F3F3" class="bodytext31" width="6%"><input type="text" name="cardname" id="cardname" size="8" style="border: 1px solid #001E6A; text-align:left;">
              ...
              <tr>
                ...
                bgcolor="#CCCCCC" class="bodytext31"><div align="right"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif">
				  ...
				  <input name="Submit2223" type="submit" value="Save Bill(Alt+S)" accesskey="s" class="button"/>
