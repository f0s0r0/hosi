...
if($employeesearch != '')
{
$query6 = "select payrollstatus from master_employee where employeecode = '$employeesearch'";
$exec6 = mysql_query($query6) or die ("Error in Query6".mysql_error());
$res6 = mysql_fetch_array($exec6);
$payrollstatus = $res6['payrollstatus'];
if($payrollstatus == 'Active')
{
$query77 = "select auto_number, typecode, componentname, amounttype, formula from master_payrollcomponent where recordstatus <> 'deleted'";
...
while($res77 = mysql_fetch_array($exec77))
{
	...
	$amounttype = $res77['amounttype'];
	
	$query2 = "select auto_number, employeecode, employeename, `$componentanum` as componentvalue from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
	...
	{
		$formulafrom = $res77['formula'];
		if($formulafrom == '1')
		{
			$query4 = "select `$formulafrom` as componentvalue from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
			...
			$query86 = "select notexceed from master_payrollcomponent where auto_number = '$componentanum'";
		...
		{	
			...
			for($i=1;$i<=2;$i++)
			{
				$query5 = "select `$i` as componentamount from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
		...
		{
			$totalgrossper = '0';
			$query77 = "select auto_number from master_payrollcomponent where typecode = '10' and recordstatus <> 'deleted'";
			$exec77 = mysql_query($query77) or die ("Error in Query77".mysql_error());
			while($res77 = mysql_fetch_array($exec77))
			{
				$auto_number = $res77['auto_number'];
	...
	if($rate != '')
	{
		...
		{	
			for($j=1;$j<=2;$j++)
			{
				$query51 = "select `$j` as componentamount from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
		...
		{
			$totalgrossper1 = '0';
			$query77 = "select auto_number from master_payrollcomponent where typecode = '10' and auto_number NOT IN ('3','4') and recordstatus <> 'deleted'";
			$exec77 = mysql_query($query77) or die ("Error in Query77".mysql_error());
			while($res77 = mysql_fetch_array($exec77))
			{
				$auto_number = $res77['auto_number'];
	...
	if($totaldays != '')
	{
		...
		{	
			for($k=1;$k<=2;$k++)
			{
				$query55 = "select `$k` as componentamount from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
		...
		{
			$query56 = "select `1` as componentamount from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
	...
	{	
		...
		$query88 = "select excludenssf, employeename from master_employee where employeecode = '$employeesearch'";
	...
	{
		$totalgrossnssf1 = '';
		$query7851 = "select auto_number from master_payrollcomponent where typecode = '10' and auto_number NOT IN ('3','4') and recordstatus <> 'deleted'";
		$exec7851 = mysql_query($query7851) or die ("Error in Query7851".mysql_error());
		while($res7851 = mysql_fetch_array($exec7851))
		{
			$auto_number = $res7851['auto_number'];
			
			$query785 = "select `$auto_number` as componentamount from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted'";
...
include("autopayecalculation1.php");
