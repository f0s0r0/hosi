...
<script type="text/javascript" src="js/autocoasearchsupplier.js"></script>
...
function escapekeypressed(e)
{
	...
	if(key == '13') //up arrow key press
	{
		...
		if (!isNaN(varActiveElementNumber)) //to prevent losing focus to submit2.
		{
			...
			//alert (varCustomerNameCatch);
			window.opener.document.getElementById("paynowlabcoa"+i+"").value = varCoaCatch;
			window.opener.document.getElementById("paynowlabtype"+i+"").value = varTypeCatch;
			window.opener.document.getElementById("paynowlabcode"+i+"").value = varIdCatch;
				window.opener.document.getElementById("paynowlabcoa"+i+"").focus();
