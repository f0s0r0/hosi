...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			if ($accountsmain != '')
			{
			$query2 = "insert into master_accountsmain(accountsmain,id,ipaddress,recorddate,username) values ('$accountsmain','$id', '$ipaddress','$updatedatetime','$username')";
	...
	header("location:chartaccountsmaindataimport1.php?st=success");
}
?>
<style type="text/css">
<!--
