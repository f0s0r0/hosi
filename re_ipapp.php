...
<?php 
$query8 = "SELECT visitcode,billno,sum(totalamount) as totalrevenue,billdate,accountname,accountnameid FROM `billing_ipcreditapprovedtransaction` WHERE billno <> '' AND `billdate` BETWEEN '$ADate1' AND '$ADate2' group by billno";
$exec8 = mysql_query($query8) or die ("Error in Query8".mysql_error());
while($res8 = mysql_fetch_array($exec8))
{
$docno = $res8['billno'];
$visitcode = $res8['visitcode'];
$transactionamount = $res8['totalrevenue'];
       ...
       <tr>
        <td valign="top"><table width="600" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			{	
				...
				$query2 = "select accountssub,auto_number,tbinclude,tbledgerview from master_accountssub where auto_number IN ('18','19','20','26','21','22','17') AND accountsmain = '$parentid' and recordstatus <> 'deleted' order by accountssub";
	...
	if($parentid != '')
	{
		if($parentid == '18' || $parentid == '19' || $parentid == '20')
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(totalamount) as paylater FROM `billing_ipcreditapprovedtransaction` WHERE `accountnameid` = '$id' AND `billdate` BETWEEN '$ADate1' AND '$ADate2'";
				$execdr1 = mysql_query($querydr1bnk) or die ("Error in querydr1bnk".mysql_error());
				while($resdr1 = mysql_fetch_array($execdr1))
				{
				$i = $i+1;
		...
		else if($parentid == '17' || $parentid == '21' || $parentid == '22')
		{
			$query267 = "select accountname,auto_number,id from master_accountname where accountssub = '$parentid'";
			$exec267 = mysql_query($query267) or die ("Error in Query267".mysql_error());
			while($res267 = mysql_fetch_array($exec267))
		...
		else if($parentid == '1')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT SUM(`amount`) as income FROM `billing_ipadmissioncharge` WHERE docno LIKE '%IPFCA%' AND recorddate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`amount`) as income FROM `billing_ipambulance` WHERE docno LIKE '%IPFCA%' AND recorddate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`amount`) as income FROM `billing_ipbedcharges` WHERE docno LIKE '%IPFCA%' AND recorddate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`labitemrate`) as income FROM `billing_iplab` WHERE billnumber LIKE '%IPFCA%' AND billdate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`amount`) as income FROM `billing_ipmiscbilling` WHERE docno LIKE '%IPFCA%' AND recorddate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`amount`) as income FROM `billing_iphomecare` WHERE docno LIKE '%IPFCA%' AND recorddate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`amount`) as income FROM `billing_ippharmacy` WHERE billnumber LIKE '%IPFCA%' AND billdate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`amount`) as income FROM `billing_ipprivatedoctor` WHERE docno LIKE '%IPFCA%' AND recorddate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`radiologyitemrate`) as income FROM `billing_ipradiology` WHERE billnumber LIKE '%IPFCA%' AND billdate BETWEEN '$ADate1' AND '$ADate2'
							UNION ALL SELECT SUM(`servicesitemrate`) as income FROM `billing_ipservices` WHERE billnumber LIKE '%IPFCA%' AND billdate BETWEEN '$ADate1' AND '$ADate2'";
		...
		else if($parentid == '28')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT (-1*sum(`transactionamount`)) as income FROM `master_transactionipdeposit` WHERE visitcode IN (select visitcode FROM `billing_ipcreditapprovedtransaction` WHERE billdate BETWEEN '$ADate1' AND '$ADate2' group by billno)
							UNION ALL SELECT (-1*sum(`transactionamount`)) as income FROM `master_transactionadvancedeposit` WHERE visitcode IN (select visitcode FROM `billing_ipcreditapprovedtransaction` WHERE billdate BETWEEN '$ADate1' AND '$ADate2' group by billno)
							UNION ALL SELECT SUM(amount) as income FROM `deposit_refund` WHERE visitcode IN (select visitcode FROM `billing_ipcreditapprovedtransaction` WHERE billdate BETWEEN '$ADate1' AND '$ADate2')";
			$execcr1 = mysql_query($querycr1bnk) or die ("Error in querycr1bnk".mysql_error());
			while($rescr1 = mysql_fetch_array($execcr1))
			{
			$i = $i+1;
		...
		else if($parentid == '27')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT SUM(-1*`rate`) as income FROM `ip_discount` WHERE patientvisitcode IN (select visitcode FROM `billing_ipcreditapprovedtransaction` WHERE billdate BETWEEN '$ADate1' AND '$ADate2' group by billno)";
			$execcr1 = mysql_query($querycr1bnk) or die ("Error in querycr1bnk".mysql_error());
			while($rescr1 = mysql_fetch_array($execcr1))
			{
			$i = $i+1;
		...
		else if($parentid == '26')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT SUM(-1*ABS(amount)) as income FROM `billing_ipnhif` WHERE visitcode IN (select visitcode FROM `billing_ipcreditapprovedtransaction` WHERE billdate BETWEEN '$ADate1' AND '$ADate2' group by billno)";
			$execcr1 = mysql_query($querycr1bnk) or die ("Error in querycr1bnk".mysql_error());
			while($rescr1 = mysql_fetch_array($execcr1))
			{
			$i = $i+1;
