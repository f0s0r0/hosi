...
{
    $insurancecompanyanum = $_REQUEST['insurancecompanyanum'];
	...
	if ($length<=100)
	{
	$query2 = "select * from master_insurancecompany where auto_number = '$insurancecompanyanum'";
    $exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
	$res2 = mysql_num_rows($exec2);
	if ($res2 != 0)
	{
		$query1 = "update master_insurancecompany set insurancecompany = '$insurancecompany', recorddate = '$updatedatetime' where auto_number = '$insurancecompanyanum'";
		$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
		$errmsg = "Success. New Insurance Company Updated.";
		//$bgcolorcode = 'success';
		header ("location:editinsurancecompany1.php?bgcolorcode=success&&st=edit&&anum=$insurancecompanyanum");
	}
	//exit();
	else
	{
		...
		//$bgcolorcode = 'failed';
		header ("location:editinsurancecompany1.php?bgcolorcode=failed&&st=edit&&anum=$insurancecompanyanum");
	}
	}
	else
	{
...
if ($st == 'edit' && $anum != '')
{
    $query1 = "select * from master_insurancecompany where auto_number = '$anum'";
	$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
	$res1 = mysql_fetch_array($exec1);
    $res1autonumber = $res1['auto_number'];
    $res1insurancecompany = $res1['insurancecompany'];
   }
    ...
    <td width="97%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="860"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebackgroundcolor1">
            <tr>
              <td><form name="form1" id="form1" method="post" action="editinsurancecompany1.php" onSubmit="return addward1process1()">
                  <table width="600" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                    <tbody>
                      ...
                      <tr>
						...
						<input name="insurancecompany" id="insurancecompany" value="<?php echo $res1insurancecompany ?>" style="border: 1px solid #001E6A; text-transform:uppercase;" size="40" /></td>
                      </tr>
                      <tr>
						...
						<input type="hidden" name="insurancecompanyanum" id="insurancecompanyanum" value="<?php echo $res1autonumber ?>">
