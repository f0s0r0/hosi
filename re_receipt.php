...
<tr>
<td colspan="12" align="center" valign="middle"><strong>Receipt Reconcile</strong></td>
</tr>
...
<?php 
$query8 = "SELECT docnumber,transactionamount,cashamount,chequeamount,cardamount,onlineamount,creditamount,transactiondate,bankcode,cashcode,mpesacode FROM `receiptsub_details` WHERE `transactiondate` BETWEEN '$ADate1' AND '$ADate2' order by auto_number";
...
<table>
 ...
 <td width="54%" valign="top"><table width="600" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			{	
				...
				if (isset($_REQUEST["ADate2"])) { $ADate2 = $_REQUEST["ADate2"]; } else { $ADate2 = ""; }
				if (isset($_REQUEST["location"])) { $location = $_REQUEST["location"]; } else { $location = ""; }
				$query2 = "select accountssub, auto_number,tbinclude,tbledgerview from master_accountssub where auto_number = '2' and accountsmain = '$parentid' and recordstatus <> 'deleted' order by accountssub";
				$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
				while($res2 = mysql_fetch_array($exec2))
				{  
					$accountsmain2 = $res2['accountssub'];
	...
	if($parentid != '')
	{
		if($parentid == '18' || $parentid == '19' || $parentid == '20')
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(`cashamount`) as bankcredit FROM `receiptsub_details` WHERE (`cashcode` = '$id') AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
					UNION ALL SELECT SUM(`creditamount`) as bankcredit FROM `receiptsub_details` WHERE (`mpesacode` = '$id') AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
					UNION ALL SELECT SUM(`cardamount`) as bankcredit FROM `receiptsub_details` WHERE (`bankcode` = '$id') AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
					UNION ALL SELECT SUM(`onlineamount`) as bankcredit FROM `receiptsub_details` WHERE (`bankcode` = '$id') AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
					UNION ALL SELECT SUM(`chequeamount`) as bankcredit FROM `receiptsub_details` WHERE (`bankcode` = '$id') AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'";
				$execdr1 = mysql_query($querydr1bnk) or die ("Error in querydr1bnk".mysql_error());
				while($resdr1 = mysql_fetch_array($execdr1))
				{
				$i = $i+1;
				$drresult[$i] = $resdr1['bankcredit'];
				}
		...
		else if($parentid == '2')
		{
			...
			{  
				...
				$i = 0;
				$crresult = array();
				$querycr1bnk = "SELECT SUM(`transactionamount`) as expenses FROM `receiptsub_details` WHERE `receiptcoa` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'";
				$execcr1 = mysql_query($querycr1bnk) or die ("Error in querycr1bnk".mysql_error());
				while($rescr1 = mysql_fetch_array($execcr1))
				{
				$i = $i+1;
				$crresult[$i] = $rescr1['expenses'];
				}
				
				$sumbalance = $sumbalance + array_sum($crresult);
			}	
			return $sumbalance;
		}
		else
