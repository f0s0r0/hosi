    ...
    <td colspan="2" valign="top"><table width="99%" border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		<table width="1202" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
          <tbody>
            <tr bgcolor="#011E6A">
              <td colspan="4" bgcolor="#CCCCCC" class="bodytext3"><strong> Trial Balance Bank</strong></td>
              </tr>
             <tr>
	...
	if($parentid != '' && $tbinclude != '')
	{
		...
		{  
			...
			if($id != '')
			{
				$i = 0;
				$drresult = array();
				$querydr1in = "SELECT SUM(transactionamount) as bankamount FROM `master_transactionpaylater` WHERE `bankcode` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `docno` LIKE 'AR-%' AND `transactionstatus` = 'onaccount' AND `transactionmodule` = 'PAYMENT'
				...
				$j = 0;
				$crresult = array();  
				$querycr1in = "SELECT SUM(creditamount) as bankcredit FROM `bankentryform` WHERE `frombankid` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
								UNION ALL SELECT SUM(-1*`transactionamount`) as bankcredit FROM `expensesub_details` WHERE `expensecoa` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
								UNION ALL SELECT SUM(-1*`transactionamount`) as bankcredit FROM `receiptsub_details` WHERE `receiptcoa` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
								UNION ALL SELECT SUM(transactionamount) as bankcredit FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `docno` LIKE 'AR-%' AND `transactionstatus` = 'onaccount' AND `transactionmodule` = 'PAYMENT'
								...
								UNION ALL SELECT SUM(`transactionamount`) as payablesdr FROM `master_transactionpharmacy` WHERE `suppliercode` = '$id' AND `transactionmodule` = 'PAYMENT' AND `recordstatus` = 'allocated' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
