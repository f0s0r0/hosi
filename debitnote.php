...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$paynowbillprefix = 'IPDr-';
$paynowbillprefix1=strlen($paynowbillprefix);
$query2 = "select * from ip_debitnote order by auto_number desc limit 0, 1";
...
$totalamount=0;
	foreach($_POST['referal'] as $key=>$value){	
		...
		$referalquery1=mysql_query("insert into ip_debitnotebrief(docno,patientcode,patientname,patientvisitcode,description,rate,billtype,accountname,consultationdate,paymentstatus,consultationtime,username,ipaddress,locationname,locationcode,unit,amount,accountcode,selaccountname)values('$billnumbercode','$patientcode','$patientfullname','$visitcode','$pairvar','$pairvar1','$billtype','$account','$dateonly','pending','$timeonly','$username','$ipaddress','".$locationnameget."','".$locationcodeget."','$units','$amount','$accountcode','$accountname')") or die(mysql_error());
		}
		
		}
		
		mysql_query("insert into ip_debitnote(billno,patientname,patientcode,visitcode,totalamount,billdate,accountname,subtype,remarks,locationname,locationcode,accountnameano,accountnameid,subtypeano,patienttype)values('$billnumbercode','$patientfullname','$patientcode','$visitcode','$totalamount','$currentdate','$account','$subtype','$remarks','".$locationnameget."','".$locationcodeget."','$accountnameano','$accountnameid','$subtypeano','$oporip')") or die(mysql_error());
		
		$query83="insert into master_transactionpaylater(patientname,patientcode,visitcode,transactiondate,accountname,billnumber,ipaddress,companyanum,companyname,financialyear,transactiontype,paymenttype,subtype,transactionamount,doctorname,locationname,locationcode,accountnameano,accountnameid,subtypeano,username,transactiontime)values('$patientfullname',
	          '$patientcode','$visitcode','$dateonly','$account','$billnumbercode','$ipaddress','$companyanum','$companyname','$financialyear','finalize','$patienttype1','$subtype','$totalamount','$doctorname','".$locationnameget."','".$locationcodeget."','$accountnameano','$accountnameid','$subtypeano','$username','$timeonly')";
	    $exec83=mysql_query($query83) or die("error in query83".mysql_error());		  

		
		header("location:debitnotelist.php?billno=".$billnumbercode."&&visitcode=".$visitcode."&&patientcode=".$patientcode."");
		exit;
...
$resy53 = mysql_fetch_array($execy53);
 //$locationnameget1 = $resy53['locationname'];
 $locationcodeget = $resy53['locationcode'];
 
 	 $queryloc="select locationname from master_location where locationcode='$locationcodeget'";
	$execloc = mysql_query($queryloc) or die("error in queryloc".mysql_error());
	$resloc = mysql_fetch_array($execloc);
	 $locationnameget = $resloc['locationname'];
...
*/
var finalbillamount = document.getElementById("insertrow4").getElementsByTagName("tr").length;
...
<body>
<form name="form1" id="frmsales" method="post" action="debitnote.php" onKeyDown="return disableEnterKey(event)" onSubmit="return validcheck()">
  ...
  <tr>
	...
	<!--	<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
				        ...
				        <tr>
				   <td colspan="13" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><strong> Debit Note </strong> </span></td>
		        </tr>
				<tr>
		...
		<tr>
		 ...
		 <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#CCCCCC" class="bodytext3"><span class="bodytext32"><strong>IP Debit</strong> </span></td>
		        </tr>
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
            bordercolor="#666666" cellspacing="0" cellpadding="2" width="99%" 
            align="left" border="0">
          <tbody id="foo">
				...
				<tr id="reffid">
                   ...
                   <input name="searchaccountname" type="text" value="" id="searchaccountname" size="30"></td>
