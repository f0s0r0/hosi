...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$categoryname= $splitdelimiter[2];
			$categoryname = strtoupper($categoryname);
			//$sampletype= $splitdelimiter[4];
			$sampletype = '';
			//$itemname_abbreviation = $splitdelimiter[5];
			$itemname_abbreviation = 'SERVICES';
			//$referencename= $splitdelimiter[6];
			$referencename = '';
			$packname= $splitdelimiter[4];
			$packname = strtoupper($packname);
			//$referencerange= $splitdelimiter[8];
			$referencerange = '';
			$rateperunit = $splitdelimiter[5];
			$rate2 = $splitdelimiter[3];
			$taxanum = $splitdelimiter[6];
			$purprice = $splitdelimiter[7];
			$genname = $splitdelimiter[8];
			$genname = strtoupper($genname);
			$ipmarkup = $splitdelimiter[9];
			$spmarkup = $splitdelimiter[10];
			$formula = $splitdelimiter[11];
			...
			$locationmed = $location.'_rateperunit';
			
			if ($itemcode != '' && $location!='')
			{
				$query89 = "select * from master_medicine where itemcode = '$itemcode'";
				$exec89 = mysql_query($query89) or die ("Error in Query89".mysql_error());
				$row89 = mysql_num_rows($exec89);
				if($row89 == 0)
				{	
					$query2 = "INSERT INTO master_medicine(itemcode, itemname, categoryname, unitname_abbreviation, rateperunit, taxanum, ipaddress, updatetime, description, purchaseprice, packageanum, packagename, formula, genericname, ipmarkup, spmarkup) 
					VALUES ('$itemcode','$itemname','$categoryname','','$rateperunit','$taxanum','$ipaddress','$updatedatetime','','$purprice','','$packname','$formula','$genname','$ipmarkup','$spmarkup')";
					$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
					
					$locationcolumn=$location.'_rateperunit';
					...
					$query78 = "INSERT INTO master_itempharmacy(itemcode, itemname, categoryname, unitname_abbreviation, rateperunit, taxanum, ipaddress, updatetime, description, purchaseprice, packageanum, packagename, formula, genericname, ipmarkup, spmarkup) 
					VALUES ('$itemcode','$itemname','$categoryname','','$rateperunit','$taxanum','$ipaddress','$updatedatetime','','$purprice','','$packname','$formula','$genname','$ipmarkup','$spmarkup')";
	...
	header("location:pharmacydataimport.php?st=success");
